    ...
    if ($Cadeau) {
        ...
        if ($Skill_Pts >= $CT) {
            ...
            if (!$ok) {
                ...
                mail(EMAIL_LOG, 'Aube des Aigles: Ajout Skill Pilote Error', $msg);
            }
            $mes = "Votre pilote reçoit sa nouvelle compétence!";
            $img = '<img src="images/promo' . $country . '.jpg">';
        } else
            $mes = "Vous n'avez pas suffisamment de points pour choisir cette compétence!<br>".Output::linkBtn('points_skills.php', 'Retour');
        $titre = "Compétence";
        $menu = Output::linkBtn('index.php?view=user', 'Retour au profil');
