...
{			
	...
	if($Division >0 and $Position !=11)
	{
		...
		if(!$Mission_Type_D)
			$Mission_Type_D_txt="<i>Ind�fini</i>";
		else
		...
		{
			$choix1="<option value='1'>Appui rapproch�</option>";
			$choix1.="<option value='11'>Attaque navale</option>";
			$choix2="<option value='12'>Bombardement naval</option>";
		...
		if($BaseAerienne >0)
		{
			$choix6.="<option value='23'>Ravitaillement a�rien</option>";
			if($para)$choix6.="<option value='24'>Parachutage</option>";
		}
		if($Date_Campagne >'1944-01-01')
			$Muns_txt="<option value='37'>APDS (AT)</option><option value='38'>HEAT (Soutien courte port�e)</option>";
		elseif($Date_Campagne >'1941-01-01')
			$Muns_txt="<option value='38'>HEAT (Soutien courte port�e)</option>";			
		$Telephone = true;						
		...
		if($Bat_Veh_Nbr >0)
		{
			if($Faction >0)
			{
				$output="<h2>Rapport � l'�tat-major</h2><form action='index.php?view=ground_appui1' method='post'>
								<input type='hidden' name='reset' value='6'>
								<input type='hidden' name='Officier' value='".$OfficierID."'>
								Signaler la situation logistique de votre bataillon � votre �tat-major.  <input type='Submit' value='RAPPORT' class='btn btn-warning' onclick='this.disabled=true;this.form.submit();'></form> Date du dernier rapport envoy� : ".$Rapport;
				$output.="<h2>Demande de renforts</h2><form action='index.php?view=ground_appui1' method='post'>
							...
							Signaler � votre Etat-Major que vous avez besoin d'aide.  <input type='Submit' value='HELP' class='btn btn-danger' onclick='this.disabled=true;this.form.submit();'></form>";
				$con=dbconnecti();
				...
				$output.="<h2>Demande d'appui a�rien</h2><form action='index.php?view=ground_appui1' method='post'>
					<input type='hidden' name='Officier' value='".$OfficierID."'>
					<input type='hidden' name='Cible' value='".$Lieu."'>
			...
			if($Ravit_Off_txt !="")
			{
				function GetRavMenu($Prefix)
				{
					...
					<option value='34'>APHE (Perforant l�ger + Explosif)</option>
					<option value='36'>APCR (Perforant courte port�e)</option>".$Muns_txt."
					</select></td></tr>";
				}
				...
				$output.="<h2>Demande de ravitaillement</h2><form action='index.php?view=ground_appui1' method='post'>
					...
					<table class='table'><thead><tr><th>Lieu de livraison</th><th>Officier</th><th>Compagnie</th><th>Chargement</th><th>Quantit�</th><th>Type de Munitions</th></thead>			
				...
				$output.="<p><b>Demande de ravitaillement impossible.</b><br>Aucun officier ravitailleur n'est pr�sent sur votre front!</p>";
			//Navires
				...
				if(($Port >0 and $Placement ==4 and $Faction_occupant_port ==$Faction) or ($Plage >0 and $Placement ==11))
				{
					...
					if($choix)
					{
						...
						$carte_txt="carte_ground.php?map=".$Front."&mode=1";
						 $output.="<h2>Demande de transport maritime <a href='help/aide_transit.php' target='_blank' title='Attention, cette action immobilisera votre unit� le temps du transit!'><img src='images/help.png'></a></h2>
						 <form action='index.php?view=ground_appui1' method='post'>
							...
							</table><div class='alert alert-danger'>Attention, cette action immobilisera votre unit� le temps du transit! Prenez contact avec le joueur contr�lant les navires AVANT de valider!</div><input type='Submit' value='VALIDER' class='btn btn-danger' onclick='this.disabled=true;this.form.submit();'></form>";
					}
				}
			}
			elseif($g_mobile ==5 and $Credits >=4 and !$Transit)
			{
				if(($Port >0 and $Placement ==4 and $Faction_occupant_port == $Faction) or ($Plage >0 and $Placement ==8))
				{
					...
					{			
						...
						$output.="<h2>Transport naval <img src='/images/CT4.png' title='Montant en Cr�dits Temps que n�cessite cette action'></h2><form action='index.php?view=ground_appui1' method='post'>
							<input type='hidden' name='Officier' value='".$OfficierID."'>
							<input type='hidden' name='reset' value='50'>
							<table class='table'><thead><tr><th>Troupes � embarquer dans ce port <a href='#' class='popup'><img src='images/help.png'><span>Vous ne pouvez embarquer que des troupes de votre faction et de votre front</span></a></th></tr></thead>
						...
						$output.="<p><b>Demande de transit impossible.</b><br>Vous devez poss�der au moins 4 barges de transport vides pour transporter un bataillon</p>";
				}
			}
			//Navires End
			//Paras
			if($para and $Credits >=1)
			{
				...
				{
					$carte_txt="carte_ground.php?map=".$Front."&mode=1";
					$output.="<h2>Demande de parachutage <a href='help/aide_transit.php' target='_blank' title='Attention, cette action immobilisera votre unit� le temps du transit!'><img src='images/help.png'></a></h2>
					 <form action='index.php?view=ground_appui1' method='post'>
						<input type='hidden' name='Officier' value='".$OfficierID."'>
						...
						<table class='table'>
							...
							<td><a href='".$carte_txt."' class='btn btn-primary' onclick='window.open(this.href); return false;'>Voir la carte</a></td></tr>
						</table><p class='lead'>Attention, cette action immobilisera votre unit� le temps du transit! Prenez contact avec le joueur contr�lant l'unit� de transport AVANT de valider!</p><input type='Submit' value='VALIDER' class='btn btn-warning' onclick='this.disabled=true;this.form.submit();'></form>";
				}
			}
			//Paras End
			//Train
			if($g_mobile !=4 and $g_mobile !=5 and $Credits >=1 and $Placement ==3 and !$Enis_IA_zone)
			{
				...
				{
					$carte_txt="carte_ground.php?map=".$Front."&mode=1";
					 $output.="<h2>Demande de transport ferroviaire <a href='help/aide_transit.php' target='_blank' title='Attention, cette action immobilisera votre unit� le temps du transit!'><img src='images/help.png'></a></h2>
					 <form action='index.php?view=ground_appui1' method='post'>
						<input type='hidden' name='Officier' value='".$OfficierID."'>
						...
						<table class='table'>
							...
							<tr><td><select name='Cible' class='form-control' style='width:50%'><option value='0'>Aucun</option>".$choix."</select></td>
							<td><a href='".$carte_txt."' class='btn btn-primary' onclick='window.open(this.href); return false;'>Voir la carte</a></td></tr>
						</table><p class='lead'>Attention, cette action immobilisera votre unit� le temps du transit! Prenez contact avec le joueur contr�lant le train AVANT de valider!</p><input type='Submit' value='VALIDER' class='btn btn-warning' onclick='this.disabled=true;this.form.submit();'></form>";
				}
			}
			...
			{		
				...
				{			
					...
					$output.="<h2>Transport ferroviaire <img src='/images/CT24.png' title='Montant en Cr�dits Temps que n�cessite cette action'>
					<a href='help/aide_transit.php' target='_blank' title='Attention, cette action immobilisera votre unit� le temps du transit!'><img src='images/help.png'></a></h2><form action='index.php?view=ground_appui1' method='post'>
						<input type='hidden' name='Officier' value='".$OfficierID."'>
					...
					$output.="<p>Vous devez poss�der une locomotive en �tat de marche, ainsi que 8 wagons de transport de troupes pour transporter un bataillon</p>";
			}
		}
		else
			$output.="<p>Votre bataillon est d�cim�!</p>";
		echo $output;
	}
	else
	{
		...
		echo "<div class='alert alert-danger'>Votre nation doit �tre en guerre et <b>vous devez faire partie d'une division ou d'une flotte</b> pour b�n�ficier de ce service!<br>Si votre unit� se trouve <b>en transit</b>, les transmissions ne sont pas accessibles.</div>";
	}
}*/
?>