...
if($ground_em_ia_naval == true) //Navire
{
    ...
    if(!$Regiment->Move and $Lieu->Meteo >-75 and $Regiment->Lieu_ID and $Autonomie >0) //Déplacement
    {
        if($Regiment->Position !=25)
        {
            $con=dbconnecti();
            $resultsmoke=mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Regiment_IA as r,Pays as p,Cible as c WHERE r.Pays=p.ID AND r.Vehicule_ID=c.ID AND p.Faction='$Faction' AND r.Lieu_ID='$Regiment->Lieu_ID' AND r.Position=37 AND c.Arme_Art>0 AND r.Vehicule_Nbr>0"),0);
            //$resulti2=mysqli_query($con,"SELECT COUNT(*),MAX(c.Vitesse) FROM Regiment as r,Pays as p,Cible as c WHERE r.Pays=p.ID AND r.Vehicule_ID=c.ID AND p.Faction<>'$Faction' AND r.Lieu_ID='$Regiment->Lieu_ID' AND r.Position=27 AND c.Type>14 AND r.Vehicule_Nbr>0 AND c.Vitesse>35 AND r.HP>c.HP/2");
            $Enis_Interdiction=mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Regiment_IA as r,Pays as p,Cible as c WHERE r.Pays=p.ID AND r.Vehicule_ID=c.ID AND p.Faction<>'$Faction' AND r.Lieu_ID='$Regiment->Lieu_ID' AND r.Position=27 AND c.Type>14 AND r.Vehicule_Nbr>0 AND c.Vitesse>35 AND r.HP>c.HP/2"),0);
            mysqli_close($con);
            if($resultsmoke)$smoke_txt="<br>Des navires alliés tenteront de créer un écran de fumée pour tenter de forcer le passage.";
            if($Enis_Interdiction and $Regiment->Vehicule_Nbr >0)
            ...
            if($G_Treve or ($G_Treve_Med and $Regiment->Front ==2) or ($G_Treve_Est_Pac and ($Regiment->Front ==1 or $Regiment->Front ==4 or $Regiment->Front ==3)))$query_treve=" AND Flag='$country'";
        }
        if($Regiment->Lieu_ID ==1984 or $Regiment->Lieu_ID ==1986 or $Regiment->Lieu_ID ==1987 or $Regiment->Lieu_ID ==1988) //Cote Espagnole
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE (Port >0 OR Zone=6 OR Plage=1) AND Latitude <50 AND Longitude <-5.35 AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        elseif($Regiment->Lieu_ID ==198 or $Regiment->Lieu_ID ==199 or $Regiment->Lieu_ID ==449 or $Regiment->Lieu_ID ==459 or $Regiment->Lieu_ID ==500 or $Regiment->Lieu_ID ==507 or $Regiment->Lieu_ID ==701 or $Regiment->Lieu_ID ==750 or $Regiment->Lieu_ID ==819 or $Regiment->Lieu_ID ==1113 or $Regiment->Lieu_ID ==1180 or $Regiment->Lieu_ID ==1181 or $Regiment->Lieu_ID ==1562 or $Regiment->Lieu_ID ==2550 or $Regiment->Lieu_ID ==2854 or $Regiment->Lieu_ID ==2909 or $Regiment->Lieu_ID ==2910 or $Regiment->Lieu_ID ==2924 or $Regiment->Lieu_ID ==2925) //Mer Adriatique
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE ID IN(198,199,449,459,500,506,507,701,750,819,1113,1180,1181,1562,2550,2854,2909,2910,2924,2925) AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        elseif($Regiment->Lieu_ID ==269 or $Regiment->Lieu_ID ==494 or $Regiment->Lieu_ID ==593 or $Regiment->Lieu_ID ==731 or $Regiment->Lieu_ID ==942 or $Regiment->Lieu_ID ==1123 or $Regiment->Lieu_ID ==1609 or $Regiment->Lieu_ID ==2302 or $Regiment->Lieu_ID ==2551) //Mer Irlande
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE ID IN(269,494,495,593,731,942,1123,1154,1609,2302,2551) AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        elseif($Regiment->Lieu_ID ==2026 or $Regiment->Lieu_ID ==2016 or $Regiment->Lieu_ID ==2011 or $Regiment->Lieu_ID ==2030 or $Regiment->Lieu_ID ==2031 or $Regiment->Lieu_ID ==2027 or $Regiment->Lieu_ID ==2028) //Corne Afrique
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >32 AND Longitude <46 AND Latitude <30 AND (Zone=6 OR Port >0 OR Plage=1) AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        elseif($Regiment->Lieu_ID ==279 or $Regiment->Lieu_ID ==280 or $Regiment->Lieu_ID ==295 or $Regiment->Lieu_ID ==296 or $Regiment->Lieu_ID ==312 or $Regiment->Lieu_ID ==470 or $Regiment->Lieu_ID ==488 or $Regiment->Lieu_ID ==489 or $Regiment->Lieu_ID ==490 or $Regiment->Lieu_ID ==592 or $Regiment->Lieu_ID ==729 or $Regiment->Lieu_ID ==730 or $Regiment->Lieu_ID ==880 or $Regiment->Lieu_ID ==952 or $Regiment->Lieu_ID ==1138 or $Regiment->Lieu_ID ==1364 or $Regiment->Lieu_ID ==1614 or $Regiment->Lieu_ID ==2540 or $Regiment->Lieu_ID ==3386) //Pas de Mer Irlande depuis l'est de l'angleterre
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE ((Longitude >0.53 AND Longitude <14 AND Latitude >44 AND Latitude <51.39) OR (Longitude >-1.62 AND Longitude <14 AND Latitude >=51.39 AND Latitude <55) OR (Longitude >-4 AND Longitude <14 AND Latitude >44 AND Latitude >=55)) AND (Port >0 OR Zone=6 OR Plage=1) AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        elseif($Regiment->Lieu_ID ==912) //Port-Saïd vers Med-Est et Suez
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >27 AND Latitude <35 AND (Port >0 OR Zone=6 OR Plage=1) AND ID NOT IN ('$Regiment->Lieu_ID',2016,2026) ORDER BY Nom ASC";
        elseif($Regiment->Lieu_ID ==2015) //Suez vers Port-Saïd et Mer Rouge
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >32 AND Latitude <31.3 AND (Port >0 OR Zone=6 OR Plage=1) AND ID NOT IN ('$Regiment->Lieu_ID') ORDER BY Nom ASC";
        elseif($Regiment->Front ==2)
        {
            if($Lieu->Longitude <15.6 and $Lieu->Latitude <45.3) //Pas de Mer Adriatique si on vient de l'ouest du détroit de Messine
                $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >-5.51 AND Longitude <73 AND Latitude <44.5 AND (Port >0 OR Zone=6 OR Plage=1) AND ID NOT IN ('$Regiment->Lieu_ID',198,199,449,458,459,500,505,506,507,701,750,819,1113,1180,1181,1562,2550) ORDER BY Nom ASC";
            elseif($country ==2 or $country ==5 or $country ==7){
                if($Lieu->Latitude >30.35)
                    $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >-5.51 AND Longitude <40 AND Latitude <44.5 AND (Port >0 OR Zone=6 OR Plage=1) AND ID NOT IN ('$Regiment->Lieu_ID',262,2015,2026) ORDER BY Nom ASC";
                else
                    $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >-5.51 AND Longitude <73 AND Latitude <44.5 AND (Port >0 OR Zone=6 OR Plage=1) AND ID NOT IN ('$Regiment->Lieu_ID',262) ORDER BY Nom ASC";
            }
            else
                $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >-5.51 AND Longitude <36 AND Latitude <44.5 AND (Port >0 OR Zone=6 OR Plage=1) AND ID NOT IN ('$Regiment->Lieu_ID',262,2015,2016,2026) ORDER BY Nom ASC";
        }
        elseif($Regiment->Front ==1)
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >13 AND Longitude <45 AND Latitude >41 AND Latitude<=50.5 AND (Port >0 OR Zone=6 OR Plage=1)".$query_treve." AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        elseif($Regiment->Front ==4)
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >13 AND Longitude <45 AND Latitude >50.5 AND (Port >0 OR Zone=6 OR Plage=1)".$query_treve." AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        elseif($Regiment->Front ==5)
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >-50 AND Longitude <60 AND Latitude >60 AND (Port >0 OR Zone=6 OR Plage=1)".$query_treve." AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        elseif($Regiment->Front ==3)
        {
            $Long_range=true;
            /*if($country ==9 and $Date_Campagne >'1941-12-01' and $Date_Campagne <'1941-12-08')
                $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >67 AND (((Port >0 OR Plage=1) AND Flag=9) OR Zone=6) AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
            else*/if($Date_Campagne <'1941-01-01')
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Flag='$country' AND Longitude >67 AND (Port >0 OR Zone=6 OR Plage=1)".$query_treve." AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        elseif(!$Faction)
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Flag='$country' AND Longitude >67 AND (Port >0 OR Zone=6 OR Plage=1)".$query_treve." AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        elseif($Lieu->Longitude <99.5)
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >67 AND Longitude <100.5 AND (Port >0 OR Zone=6 OR Plage=1)".$query_treve." AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        elseif($Lieu->Longitude >99.5)
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >99.4 AND (Port >0 OR Zone=6 OR Plage=1)".$query_treve." AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        else
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude >67 AND (Port >0 OR Zone=6 OR Plage=1)".$query_treve." AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        }
        elseif($Regiment->Lieu_ID ==344 or $Regiment->Lieu_ID ==503) //Gibraltar
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude <-5.34 AND Latitude <50 AND (Port >0 OR Zone=6 OR Plage=1) AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        elseif($Lieu->Latitude >67)
        {
            $Long_range=true;
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude <60 AND Latitude >60 AND (Zone=6 OR Port >0 OR Plage=1) AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        }
        elseif($Lieu->Longitude <-45)
        {
            $Long_range=true;
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude <-10 AND (Zone=6 OR Port >0 OR Plage=1) AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        }
        elseif($Lieu->Longitude <=-10)
        {
            if($Regiment->Lieu_ID ==1992)
                $Dist_max=800;
            else
                $Dist_max=1000;
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude <=-8 AND (Zone=6 OR Port >0 OR Plage=1) AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        }
        elseif($Lieu->Longitude <-0.5 and $Lieu->Longitude >-10 and $Lieu->Latitude >39.99 and $Lieu->Latitude <48) //Golfe de Gascogne
        {
            $Dist_max=306;
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude <-0.5 AND Latitude <48.5 AND (Port >0 OR Zone=6 OR Plage=1) AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        }
        elseif($Lieu->Longitude <-5.5 and $Lieu->Latitude <45) //Côte Ouest Afrique
        {
            $Dist_max=521;
            $query="SELECT ID,Nom,Longitude,Latitude,Impass,Port,Zone,Flag FROM Lieu WHERE Longitude <-5.51 AND (Port >0 OR Zone=6 OR Plage=1) AND ID<>'$Regiment->Lieu_ID' ORDER BY Nom ASC";
        }
        elseif($Lieu->Longitude >9 and $Lieu->Latitude >53.8) //Mer Baltique
        ...
        elseif($Lieu->Longitude >=7 and $Lieu->Longitude <=11.5 and $Lieu->Latitude >=57) //Skagerrak + Kattegat
        {
            $Dist_max=250;
        ...
        {
            if($Regiment->Lieu_ID ==3155)$Lieu_exc=730;
            elseif($Regiment->Lieu_ID ==730)$Lieu_exc=3155;
            if($Lieu->Longitude <=-8)
                $Dist_max=425;
            else
                $Dist_max=250;
        ...
        if($Long_range)
        {
            if($Regiment->Transit_Veh ==5000)
                $Dist_max=1200;
            else
                $Dist_max=$Veh->Fuel; //1200
                ...
                if($Distance[0] <=$Dist_max)
                {
                    ...
                    else
                    {
                        ...
                        if($sensh)
                        {
                            ...
                            elseif($Lieu->Latitude <$data2[3]-0.5)
                            {
                                $sensv='Nord';
                                $coord+=10;
    ...
    if($Regiment->Vehicule_ID ==5001 or $Regiment->Vehicule_ID ==5124 or $Regiment->Vehicule_ID ==5392) //cargos
    {
        $Divisions='Etat-Major';
        $Positions='En position';
        $Pos_titre='Fret';
        if($Regiment->Vehicule_ID ==5392)
            $Pos_ori="Cargaison";
        elseif(!$Regiment->Division)
        {
            if(!$Regiment->Fret)
                $Pos_ori="Vide";
        ...
        else
        {
            ...
            elseif($Regiment->Fret ==1200)
                $Pos_ori="Obus de 200mm";
            elseif($Regiment->Fret >9000 or $Regiment->Fret ==80 or $Regiment->Fret ==930 or $Regiment->Fret ==1)
                $Pos_ori="Fret incompatible";
            else
                $Pos_ori="Obus de ".$Regiment->Fret."mm";
        }
        if($Regiment->Vehicule_Nbr >0)
        {
            if($Regiment->Vehicule_ID ==5392) //Flottille ravit
            {
                $con=dbconnecti();
            ...
            if($Lieu->ValeurStrat >3 and $Faction ==$Faction_Flag and !$Regiment->Move) //Dépot
            {
                ...
                elseif($Regiment->Division and $Ordres_Div)
                {
                    ...
                    /*if($Regiment->Fret)
                    {
                        $Regs_div=false;
                        $con=dbconnecti();
                        $result9=mysqli_query($con,"SELECT r.ID FROM Regiment as r,Officier as o WHERE r.Officier_ID=o.ID AND r.Lieu_ID='$Regiment->Lieu_ID' AND o.Division='$Regiment->Division'");
                        mysqli_close($con);
                        if($result9)
                        {
                        ...
                        if($Regs_div)
                            $Decharger="<form action='index.php?view=ground_em_ia_go' method='post'><input type='hidden' name='Unit' value='".$Unit."'><input type='hidden' name='Dech' value='".$Regiment->Fret."'>
                ...
                if($Regiment->Vehicule_ID ==5124 and $Lieu->Longitude <-52 and ($country ==2 or $country ==7))
                {
                    $LLease=true;
            ...
            /*if($Regiment->Division and !$Ordres_Div)
            {
                $Lieux_txt="<select name='cible' class='form-control' style='width: 200px'><option value='0'>Rester à ".$Lieu->Nom."</option>";
            ...
            {
                $con=dbconnecti();
                $resultgr=mysqli_query($con,"SELECT r.ID,c.Nom FROM Regiment_IA as r,Cible as c WHERE r.Vehicule_ID=c.ID AND r.Lieu_ID='$Regiment->Lieu_ID' AND r.Pays='$country' AND c.Categorie IN(17,20,21,22,23,24) AND r.Autonomie < c.Autonomie");
                mysqli_close($con);
                if($resultgr)
                {
    ...
    elseif(($Ordres_Cdt or $Ordres_Mer or $Ordres_Armee or $Ordres_Div or $Ordres_Bat) and (!$GHQ or $Admin or $Nation_IA))
    {
        ...
        elseif($Veh->Type ==37)
            $Sub_IA=true;
        elseif($Veh->Type ==15 or $Veh->Type ==17)
            $ASM=true;
        elseif($Veh->Type ==14 and $Veh->Detection >10)
            $Patrouilleur=true;
        elseif($Veh->Type ==20)
            $Meteo_Move_Limit=-100;
        if($Veh->Type ==17 or $Veh->Type ==18 or $Veh->Type ==19)
            $Torp_IA=true;
        if(!$Regiment->Move and $Regiment->Vehicule_Nbr >0 and $Regiment->Position !=34 and $Autonomie >0 and $Regiment->HP >0)
        {
            if($Lieu->Meteo >$Meteo_Move_Limit)
            {
                $CT_Bomb=0; //2-$Sec_EM;
                $CT_Torp=0; //4-$Sec_EM;
                if($Sub_IA)
                ...
                if((($Torp_IA and $Regiment->CT >=$CT_Torp) or $Sub_IA) and ($Lieu->Zone ==6 or $Regiment->Placement ==8) and $Veh->Arme_AT and $HasHostiles)
                {
                    //Range
                    if($Sub_IA)
                    {
                        $con=dbconnecti();
                        $Rudels=mysqli_query($con,"SELECT COUNT(*) FROM Regiment_IA as r,Cible as c WHERE r.Vehicule_ID=c.ID AND r.Lieu_ID='$Regiment->Lieu_ID' AND r.Pays='$country' AND r.Placement=8 AND r.Vehicule_Nbr >0 AND c.Type=37");
                        mysqli_close($con);
                    }
                    if($Matos ==8)$Range/=2;
                    if($Regiment->Position ==26)
                        $Range/=2;
                    elseif($Regiment->Position ==25 or $Regiment->Position ==28)
                        $Range*=2;
                    if($Lieu->Meteo <-69)
                        $Max_Range=500;
                    elseif($Lieu->Meteo <-9)
                        ...
                        $Max_Range=2000;
                    if($Range >$Max_Range)$Range=$Max_Range;
                    if($Regiment->Placement ==4)
                        $Range=20000; //Au port tous les navires de surface peuvent être ciblés
                    elseif($Regiment->Skill ==43)
                        $Range*=(1+((5*$Rudels)/10));
                    elseif($Regiment->Skill ==168)
                        $Range*=(1+((10*$Rudels)/10));
                    elseif($Regiment->Skill ==169)
                        $Range*=(1+((15*$Rudels)/10));
                    elseif($Regiment->Skill ==170)
        ...
        elseif($Sub_IA)
        {
            if(!$Positions)
            {
                ...
                if($Regiment->Position !=34)$Positions.="<option value='25'>Plongée</option>";
        ...
        elseif($Regiment->Transit_Veh ==5000)
        {
            ...
            $barges_txt='<br>'.GetVehiculeIcon(5000,$country,0,0,$Regiment->Front,"Transporté par des barges");
            if(!$Regiment->Move and $Lieu->Zone !=6 and $Lieu->Plage and $Regiment->Placement ==8 and ($Veh->Amphi or $Faction_Plage ==$Faction))
            {
                $con=dbconnecti();
                //$Unitsb_Plage=mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Regiment as r,Pays as p WHERE r.Pays=p.ID AND p.Faction='$Faction' AND r.Lieu_ID='$Regiment->Lieu_ID' AND r.Placement=11 AND r.Vehicule_Nbr >0"),0);
                $Unitsa_Plage=mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Regiment_IA as i,Pays as y WHERE i.Pays=y.ID AND y.Faction='$Faction' AND i.Lieu_ID='$Regiment->Lieu_ID' AND i.Placement=11 AND i.Vehicule_Nbr >0"),0);
                mysqli_close($con);
        ...
        if($PA_IA or $Veh->Hydra)
        {
            ...
            {
                $con=dbconnecti();
                $result10=mysqli_query($con,"SELECT ID,Pays,Nom,Avion1,Avion2,Avion3,Avion1_Nbr,Avion2_Nbr,Avion3_Nbr,Mission_IA FROM Unit WHERE Porte_avions='$Regiment->Vehicule_ID' AND Etat=1");
                mysqli_close($con);
                if($result10)
                {
                    while($data10=mysqli_fetch_array($result10,MYSQLI_ASSOC))
                    {
                        ...
                        $depot_info.="<tr><td>".$Esc_PA_txt."</td><td>"
                            .$data10['Avion1_Nbr']."x ".GetAvionIcon($data10['Avion1'],$data10['Pays'],0,$data10['ID'],$Regiment->Front)."</td><td>"
                            .$data10['Avion2_Nbr']."x ".GetAvionIcon($data10['Avion2'],$data10['Pays'],0,$data10['ID'],$Regiment->Front)."</td><td>"
                            .$data10['Avion3_Nbr']."x ".GetAvionIcon($data10['Avion3'],$data10['Pays'],0,$data10['ID'],$Regiment->Front)."</td></tr>";
                    }
    ...
    elseif($Ordres_Adjoint and $Regiment->Transit_Veh ==5000 and (!$GHQ or $Admin or $Nation_IA))
    {
        $Faction_Port=GetData("Pays","ID",$Lieu->Flag_Port,"Faction");
        if(!$Regiment->Move and $Lieu->Port >10 and $Faction ==$Faction_Flag and $Faction ==$Faction_Port)
        {
            $con=dbconnecti();
            $Unitsa_Port=mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Regiment_IA as i,Pays as y WHERE i.Pays=y.ID AND y.Faction='$Faction' AND i.Lieu_ID='$Regiment->Lieu_ID' AND i.Placement=4 AND i.Vehicule_Nbr >0"),0);
            mysqli_close($con);
            if($Unitsa_Plage >4)
        ...
        $Positions="<select name='pos' class='form-control' style='max-width:200px; display:inline;'>
                                ...
                                <option value='22'>Evasion (navire rapide désirant éviter le combat)</option>";
        $barges_txt='<br>'.GetVehiculeIcon(5000,$country,0,0,$Regiment->Front,"Transporté par des barges");
        if($Veh->Amphi and $Lieu->Zone !=6 and $Lieu->Plage and $Regiment->Placement ==8)
        {
            $con=dbconnecti();
            //$Unitsb_Plage=mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Regiment as r,Pays as p WHERE r.Pays=p.ID AND p.Faction='$Faction' AND r.Lieu_ID='$Regiment->Lieu_ID' AND r.Placement=11 AND r.Vehicule_Nbr >0"),0);
            $Unitsa_Plage=mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Regiment_IA as i,Pays as y WHERE i.Pays=y.ID AND y.Faction='$Faction' AND i.Lieu_ID='$Regiment->Lieu_ID' AND i.Placement=11 AND i.Vehicule_Nbr >0"),0);
    ...
    //Renforts
    if(!$Regiment->Transit_Veh)
    {
        $Faction_Port=GetData("Pays","ID",$Lieu->Flag_Port,"Faction");
        if($Lieu->Port >0 and $Faction ==$Faction_Flag and $Faction ==$Faction_Port && !$Regiment->Move)
        {
            $Port_ok=false;
            if($Lieu->Port_level ==3)$Port_ok=true;
            elseif($Lieu->Port_level ==2 and ($Veh->Type <20 or $Veh->Type ==37))$Port_ok=true;
            elseif($Lieu->Port_level ==1 and ($Veh->Type ==14 or $Veh->Type ==15))$Port_ok=true;
            if($Port_ok)
            {
                $CT_cale=0;
                $con=dbconnecti();
                if($Veh->Type>17 and $Veh->Type<22)
                {
                    $coules=mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Ground_Cbt WHERE Veh_b='$Regiment->Vehicule_ID'"),0);
                    if($coules>0)$CT_cale=CT_MAX;
                }
                //$Enis_Port=mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Regiment as r,Pays as p WHERE r.Pays=p.ID AND p.Faction<>'$Faction' AND r.Lieu_ID='$Regiment->Lieu_ID' AND r.Placement=4 AND r.Vehicule_Nbr >0"),0);
                $Enis2_Port=mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Regiment_IA as i,Pays as y WHERE i.Pays=y.ID AND y.Faction<>'$Faction' AND i.Lieu_ID='$Regiment->Lieu_ID' AND i.Placement=4 AND i.Vehicule_Nbr >0"),0);
                mysqli_close($con);
                ...
                elseif(!$Regiment->Move and $Regiment->CT >=$CT_cale and (
                        ($Regiment->HP<$Veh->HP and $Regiment->HP>0) or
                        ($Regiment->Vehicule_Nbr<1 and ($Regiment->Vehicule_ID ==5001 or $Regiment->Vehicule_ID ==5124 or $Veh->Type ==37 or $Veh->Type ==14)) or
                        ($Regiment->Vehicule_Nbr<4 and ($Veh->Type ==15 or $Veh->Type ==16)) or
                        ($Regiment->Vehicule_Nbr<2 and $Veh->Type ==17)
                    ))
                    $Renforts_txt='<tr><td>
                                            <form action="index.php?view=ground_em_ia_go" method="post">
                ...
                elseif(!$Regiment->Move and $Regiment->CT ==CT_MAX and $Regiment->Vehicule_Nbr<1 and !$Regiment->HP and ($Veh->Type>17 and $Veh->Type<22))
                    $Renforts_txt='<tr><td>
                                            <form action="index.php?view=ground_em_ia_go" method="post">
        ...
        if($Veh->HP)
            $hp_good=round(($Regiment->HP/$Veh->HP)*100);
        else
            $hp_good=0;
        $hp_bad=100-$hp_good;
        if($Regiment->HP)
        ...
        if($Veh->Categorie ==20 or $Veh->Categorie ==21 or $Veh->Categorie ==22 or $Veh->Categorie ==23 or $Veh->Categorie ==24 or $Veh->Categorie ==17){
            $Autonomie_txt='<a href="#" class="popup">
                            <b class="badge">'.$Autonomie.'/'.$Veh->Autonomie.' Jours</b>
                            <span>
    ...
    if($Regiment->Front == 2 and $Lieu->Longitude <12)
        $front_carte=12;
    else
