	...
	{	
		...
		if($Lieu and !$Placement)
		{
			...
			if($Admin or $OfficierEMID)
			{
				...
				if($Admin)$Veh_txt.="V�hicule <select name='Veh' class='form-control' style='width: 200px'>".$Veh."</select>";
			}
			$con=dbconnecti();
		...
		elseif($Lieu and $Placement)
		{
				...
				if($result2)
				{
					while($data2=mysqli_fetch_array($result2,MYSQLI_ASSOC))
					{
						if($NoeudR >0 and $Placement ==2 and !$Rasputitsa and !$Mousson and !$Enis)
						{
							...
							{
								$Zone_calc=0;
								$txt_help.="<br>L'unit� se trouvant sur un noeud routier et aucun ennemi n'�tant pr�sent sur ce noeud routier, l'�ventuel malus de terrain est annul�.";
							}
							else
								$txt_help.="<br>Malgr� le fait que l'unit� se trouve sur un noeud routier et qu'aucun ennemi ne soit pr�sent sur ce noeud routier, l'�ventuel malus de terrain n'est pas annul� � cause de la Rasputitsa.";
						}
						...
						if($Skill4 ==100 and $Zone ==8) //guerre du d�sert
							$data2['Fuel']*=2;
						...
						if($result2)
						{
							while($data2=mysqli_fetch_array($result2,MYSQLI_ASSOC))
							{
								if($NoeudR >0 and $Placement ==2 and !$Rasputitsa and !$Mousson and !$Enis)
								{
									...
									{
										$Zone_calc=0;
										$txt_help.="<br>L'unit� se trouvant sur un noeud routier et aucun ennemi n'�tant pr�sent sur ce noeud routier, l'�ventuel malus de terrain est annul�. (1)";
										$txt_anex=true;
									}
									else
										$txt_help.="<br>Malgr� le fait que l'unit� se trouve sur un noeud routier et qu'aucun ennemi ne soit pr�sent sur ce noeud routier, l'�ventuel malus de terrain n'est pas annul� � cause de la Rasputitsa.";
			...
			if($Front ==2)
			{
				...
				elseif($Latitude >36.6 and $Longitude >19) //Gr�ce
			...
			elseif($Front ==1 or $Front ==4)
			{
				...
				elseif($Latitude <46 and $Latitude>44.40 and $Longitude >33 and $Longitude <36.5) //Crim�e
			...
			$txt_help.="<br>Votre unit� poss�de une autonomie de base de ".$Autonomie_Min."km.<br>L'autonomie maximale sur ce front est de ".$Autonomie_Max."km.";
			$con=dbconnecti();
			...
			if($result)
			{
				while($data=mysqli_fetch_array($result))
				{
					...
					if($Distance[0] <=$Autonomie_Actu)
					{
						...
						if($Train_move)
						{
							$txt_help .="<br>L'unit� �tant sur une gare contr�l�e par sa nation et le lieu de destination (".$data[1].") comportant �galement une gare contr�l�e par sa nation, l'autonomie de l'unit� est de ".$Autonomie_Actu."km";
						}
						elseif($Bonus_Noeud)
						{
							$txt_help.="<br>L'unit� �tant sur un noeud routier alors qu'aucune unit� ennemie n'est pr�sente sur ce noeud routier et le lieu de destination (".$data[1].") comportant �galement un noeud routier, l'autonomie de l'unit� est doubl�e pour atteindre la valeur de ".$Autonomie_Actu."km (1)";
							$txt_anex=true;
						}
						else
							$txt_help.="<br>L'unit� ne b�n�ficie pas du bonus de noeud routier vers ce lieu (".$data[1].")";
						$Impass=$data['Impass'];
						...
						if($Rasputitsa or ($Merzlota and $Mobile_Min !=3))
						{
							...
							{
								$Distance[0]*=1.25;
								$txt_help.="<br>La distance � parcourir vers ".$data[1]." est augment�e de 25% (".$Distance[0]."km) � cause de la Rasputitsa";
							}
						}
						if($Faction != GetData("Pays","ID",$data['Flag'],"Faction"))
						{
							...
							{
								$Distance[0]*=2;
								$txt_help.="<br>La distance � parcourir vers ".$data[1]." est augment�e de 100% (".$Distance[0]."km) � cause de la pr�sence d'unit�s ennemies sur un lieu contr�l� par l'ennemi. (1)";
							}
							else
							{
								$Distance[0]*=1.5;
								$txt_help.="<br>La distance � parcourir vers ".$data[1]." est augment�e de 50% (".$Distance[0]."km) car ".$data[1]." est contr�l� par l'ennemi. (1)";
							}
						}
						...
						if($Distance[0] <=$Autonomie_Actu)
						{
							$txt_help.="<br>La distance � parcourir vers ".$data[1]." est de <span class='text-primary'>".$Distance[0]."km</span> et l'autonomie de l'unit� est de ".$Autonomie_Actu.", permettant le d�placement vers ce lieu.";
							if($Credits >= $CT_city)
							...
							$txt_help.="<br>La distance � parcourir vers ".$data[1]." est de <span class='text-danger'>".$Distance[0]."km</span> et l'autonomie de l'unit� est de ".$Autonomie_Actu.", ne permettant pas le d�placement vers ce lieu.";
					}
				}
			...
			if($choix)
			{
				...
				$mes="<h1>Calculateur de distance</h1><h2>Destinations</h2>
					<p class='lead'>Autonomie max : ".$Autonomie_Min."km ".$train_txt. "<a href='#' class='popup'><img src='../images/help.png'><span>Doubl� si votre bataillon se trouve sur un noeud routier</span></a></p>
				...
				if($txt_anex)$mes.="<p class='lead'>(1) Attention que cette information ne prend pas en compte la pr�sence d'�ventuelles unit�s ennemies!</p>";
			}
			elseif($Mobile_Min ==5)
				$mes="<br>Cet outil ne permet pas de simuler le d�placement des unit�s navales que vous commandez.";
			else
