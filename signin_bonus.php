...
if($AccountID >0 and $Pays >0 and !$_SESSION['creation_off_em'])
{
	...
	if($Pays and $AccountID and $Nom and $Photo)
	{
		...
		$con=dbconnecti();
		$result=mysqli_query($con,"SELECT COUNT(*) FROM Pilote,Pilote_IA,Officier,Officier_em,Officier_bonus WHERE Nom='$Nom'");
		mysqli_close($con);
		if($result) 
		{
		...
		if(!empty($Nom) and !empty($Photo) and !empty($Pays) and !empty($AccountID))
		{
			...
			else
			{
				...
				if($ok)
				{
					...
					$query_update="UPDATE Joueur SET Officier_bonus='$ins_id' WHERE ID='$AccountID'";
					...
					else
					{
						...
						mail('binote@hotmail.com','Aube des Aigles: Nouvel Officier EM Premium',$AccountID." / Nom : ".$Nom." / Pays : ".$Pays);
	...
	<form action='index.php?view=signin_bonus' method='post'>
