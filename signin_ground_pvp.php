...
if($AccountID >0 and $Pilote >0 and $Pays >0)
{
	...
	if($Pays and $Pilote and $Nom)
	{
		...
		{
			if(!preg_match("#^[[:alpha:]�������������'\- ]+$#", $Nom) or $Pseudo_Reserve or strlen($Nom) < 7)
				echo "Le nom de votre officier n'est pas valide ou est d�j� utilis�!<br>Le nom du pilote doit comporter au moins 6 lettres, et �ventuellement un espace entre le pr�nom et le nom.";
			else
			{
				...
				if($ok)
				{
					...
					{
						mysqli_close($con);
						$mes.="Erreur de cr�ation de votre Officier!";
					}
					else
					{
						$_SESSION['Officier_pvp']=$ins_id;
						$mes.="Officier cr�� avec succ�s!";
					}
					mail('binote@hotmail.com','Aube des Aigles: Nouvel Officier PVP',$login." / Nom : ".$Nom." / Pays : ".$Pays);					
					echo "<p>Personnage cr�� avec succ�s!</p>";
					echo "<p><img src='images/transfer_yes".$Pays.".jpg'></p>";
					echo "<hr><a title='Acc�der au menu' href='index.php?view=ground_menu_pvp' class='btn btn-default'>Acc�der au menu action</a>";
					exit;
				}
				else
				{
					$mes.="Erreur de cr�ation de personnage (".$IP.") ".mysqli_error($con);
					mail('binote@hotmail.com', 'Aube des Aigles: Signin error ground pvp',$mes);
					echo "<p>Erreur de cr�ation de Personnage terrestre !</p>";
					exit;
				}
			}
		}
	...
	$titre="Cr�ation de votre officier";
	?>
	<h1><?echo $titre;?></h1>
	...
	<tr><td align="left"><input type="text" title="Le nom de l'officier ne peut comporter que des lettres et �ventuellement un espace entre le pr�nom et le nom" name="name" size="30" class='form-control' onmouseup='valbtn.disabled=false;' required></td></tr></table>
