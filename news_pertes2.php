...
if(isset($_SESSION['AccountID']) AND $PlayerID > 0)
{
	...
	{			
		...
		$i=0;
		$con = dbconnecti();
		$result = mysqli_query($con, "SELECT ID,Pays,Date,Nom FROM Cible WHERE Unit_ok=1 AND Date < '$Date_Campagne' ORDER BY Date ASC");
		mysqli_close($con);
		if($result)
		{
			while($data = mysqli_fetch_array($result, MYSQLI_ASSOC))
			{
				...
				//Transfer	
				$con = dbconnecti(4);
				$Perdus = mysqli_result(mysqli_query($con, "SELECT SUM(Avion_Nbr) FROM Events_Ground WHERE Event_Type IN (400,401,403,404,405,420,605) AND Avion='$ID'"),0);
				mysqli_close($con);
				$i++;
				echo "<tr><td>".$i."</td><td>".$Avion_img."</td><td><img src='".$Pays."20.gif'></td><td>".$data['Date']."</td><td bgcolor='MistyRose'>".$Perdus."</td></tr>";
			}
			mysqli_free_result($result);
			unset($data);
		}
