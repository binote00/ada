...
{	
	...
	if($OfficierEMID >0)
	{
		...
		{		
            ...
            $resultu=mysqli_query($con,"SELECT Base,Type,Avion1,Avion2,Avion3,Armee,Mission_IA FROM Unit WHERE ID='$Unitet'");
            if($resultu)
            {
            ...
            if(!$Mission_IA && ($OfficierEMID ==$Commandant || $OfficierEMID ==$Officier_Adjoint || $GHQ || ($Armee >0 && ($Unit_Armee ==$Armee))))
            {
