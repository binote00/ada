...
if($_SESSION['AccountID'] and ($PlayerID or $OfficierID or $OfficierEMID))
{
...
?>
	...
	</head><body><div id="dhtmltooltip"></div><script type="text/javascript" src="dhtml_tooltip.js"></script>
<?
	...
	if($result)
	{
		while($data=mysqli_fetch_array($result,MYSQLI_ASSOC))
		{
			...
			$Nom_ville="<div class=\'float\'><b>".$data['Nom']."</b></div>";
			$Pays="<div class=\'float flag".$data['Pays']."\'></div><br>";
			...
			if($Mode ==15 and ($OfficierID >0 or $OfficierEMID >0))
			{
				...
				{
					?><div class="range50 r" style="top:<?=$latit-10?>; left:<?=$longit-10?>;"></div><?
				}
			}
			...
			if($EM_access and ($Front_EM ==$Front or $Front_EM ==99))
			{
				if($Mode ==5)
				{
					...
					if($Couv_IA >0)
					{
						?><div class="range50" style="top:<?=$latit-5?>; left:<?=$longit-5?>;"></div><?
					}
					if($Couv_Nuit_IA >0)
					{
						?><div class="range50 v" style="top:<?=$latit-5?>; left:<?=$longit-5?>;"></div><?
					}
					if($Esc_IA >0)
					{
						?><div class="range50 b" style="top:<?=$latit-5?>; left:<?=$longit-5?>;"></div><?
					}
					if(!$Couv_IA and !$Couv_Nuit_IA and !$Esc_IA and $Task_IA >0)
					{
						?><div class="range50 p" style="top:<?=$latit-5?>; left:<?=$longit-5?>;"></div><?
					}
				}
				elseif($Mode ==3)
				{
				...
				elseif($Mode ==8 and !$OfficierEMID)
				{
					...
					if($Troupes >0)
					{
						if($Zone ==6)
						{
							?><div class="ships<?echo $country;?>" style="top:<?=$latit-10?>; left:<?=$longit-5?>;"></div><?
						}
						else
						{
							?><div class="troops<?echo $country;?>" style="top:<?=$latit?>; left:<?=$longit-12?>;"></div><?
						}
					}
				}
			}
				...
				if($OfficierEMID >0)
				{
					...
					if($result_unit)
					{
						...
						if($Mode ==8 and $bulle_txt_global !='' and !$icone_lock)
						{
							if($Zone ==6)
							{
								?><div onMouseover="ddrivetip('<?echo $ValStrat." <b>".$data['Nom']."</b> ".$Icone_txt.'<hr>'.$bulle_txt_global;?>', '#ECDDC1', '<?echo $Info_large;?>', '<?echo $Info_long;?>')"; onMouseout="hideddrivetip()">
								<?echo $arch_link;?><div class="ships<?echo $country;?>" style="top:<?=$latit-10?>; left:<?=$longit-5?>;"></div><?echo $arch_linkf;?></div><?
							}
							else
							{
								?><div onMouseover="ddrivetip('<?echo $ValStrat." <b>".$data['Nom']."</b> ".$Icone_txt.'<hr>'.$bulle_txt_global;?>', '#ECDDC1', '<?echo $Info_large;?>', '<?echo $Info_long;?>')"; onMouseout="hideddrivetip()">
								<?echo $arch_link;?><div class="troops<?echo $country;?>" style="top:<?=$latit?>; left:<?=$longit-12?>;"></div><?echo $arch_linkf;?></div><?
							}
						}
						unset($data_unit);
					}
	...
	if($Lieu_search)
	{
		if($iframe)
		{
			...
			<a href="javascript:void(0);" onclick="window.parent.location.href='index.php?view=em_city_ground&id=<?echo $Lieu_search;?>&mode=3';" target='_top'><div class="cible" title="<?echo $Nom_Sel;?>" style="top:<?=$Lat_Sel?>; left:<?=$Long_Sel?>;"></div></a></div><?
		}
		else
		{
			?><div onMouseover="ddrivetip('<?echo $Info_Sel;?>','#ECDDC1','200','<?echo $Info_Long_Sel;?>')"; onMouseout="hideddrivetip()">
			<a href='em_city_ground.php?id=<?echo $Lieu_search;?>' target='_blank'><div class="cible" title="<?echo $Nom_Sel;?>" style="top:<?=$Lat_Sel?>; left:<?=$Long_Sel?>;"></div></a></div><?
		}
	}?>
