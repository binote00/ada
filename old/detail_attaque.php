...
{			
	...
	if($PlayerID > 0)
	{
		...
		if($Renseignement > 100 or $Avancement > 9999 or $PlayerID == $Commandant or $PlayerID == $Officier_Adjoint or $PlayerID == $Officier_EM or $PlayerID == $Officier_Rens or $PlayerID == 1)
		{
			$ID = Insec($_POST['id']);
			$Datediff = Insec($_POST['date']);
			if(!$ID and !$Datediff)
			{
				$ID = Insec($_GET['id']);
				$Datediff = Insec($_GET['date']);
			}			
			if(!ctype_digit($ID))
				exit;
		...
		?>
		<div id="tab_menu">
		<h2>Détail de l'attaque d'il y a <?echo $Datediff;?> jours sur <?echo $Lieu_Target;?></h2>
		<ol>
		...
		<?			
				$con=dbconnecti();
				$DCA = mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM DCA WHERE Lieu = '$ID' AND DATEDIFF(DATE(`Date`),'$Dateref') = -'$Datediff'"),0);
				$Recce = mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Recce WHERE DATEDIFF(DATE(`Date`),'$Dateref') = -'$Datediff' AND Lieu='$ID'"),0);
				if($war)
				{
					$Escorte = mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Escorte WHERE Lieu = '$ID' AND DATEDIFF(DATE(`Date`),'$Dateref') = -'$Datediff'"),0);
				}
				else
				{
					$Escorte = "?";
				...
				if($Occupant == $country)
				{
					$Patrouille = mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Patrouille WHERE Lieu = '$ID' AND DATEDIFF(DATE(`Date`),'$Dateref') = -'$Datediff'"),0);
				}
				else
				{
					$Patrouille = "?";
				}
				$Attaque = mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Attaque WHERE Lieu = '$ID' AND DATEDIFF(DATE(`Date`),'$Dateref') = -'$Datediff'"),0);
				$Bombardement = mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Bombardement WHERE Lieu = '$ID' AND DATEDIFF(DATE(`Date`),'$Dateref') = -'$Datediff'"),0);
				$Chasse = mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Chasse WHERE Lieu = '$ID' AND DATEDIFF(DATE(`Date`),'$Dateref') = -'$Datediff'"),0);
				mysqli_close($con);
		?>
		<div id="tab_resume">
		...
		<table class='table table-striped'>
			<thead><tr>
				...
				$con=dbconnecti();
				$query2=mysqli_query($con,"SELECT DISTINCT * FROM Recce WHERE DATEDIFF(DATE(`Date`),'$Dateref') = -'$Datediff' AND Lieu='$ID' ORDER BY ID DESC, Joueur ASC");
				mysqli_close($con);
				if($query2)
				{
		...
		<table class='table table-striped'>
			<thead><tr>
				...
				<?
				$con=dbconnecti();
				$query2=mysqli_query($con,"SELECT DISTINCT * FROM Bombardement WHERE Lieu = '$ID' AND DATEDIFF(DATE(`Date`),'$Dateref') = -'$Datediff' ORDER BY ID DESC, Joueur ASC");
				mysqli_close($con);
				if($query2)
				{
		...
		<table class='table table-striped'>
			<thead><tr>
				...
				<?				
				$con=dbconnecti();
				$query2=mysqli_query($con,"SELECT * FROM Escorte WHERE Lieu='$ID' AND DATEDIFF(DATE(`Date`),'$Dateref')=-'$Datediff' ORDER BY ID DESC, Joueur ASC");
				mysqli_close($con);
				if($query2)
				{
					while($data2=mysqli_fetch_array($query2))
					{
						$Pays_win=GetData("Joueur","ID",$data2['Joueur'],"Pays");
						if($Pays_win ==$country)
						{
							$Date=substr($data2['Date'],0,16);
							$Joueur_win=GetData("Pilote","ID",$data2['Joueur'],"Nom");
		...
		<table class='table table-striped'>
			<thead><tr>
				...
				<?				
				$con=dbconnecti();
				$query1=mysqli_query($con,"SELECT * FROM Attaque WHERE Lieu='$ID' AND DATEDIFF(DATE(`Date`),'$Dateref')=-'$Datediff' AND Type >0 ORDER BY ID DESC, Joueur ASC");
				mysqli_close($con);
				if($query1)
				{
		...
		<table class='table table-striped'>
			<thead><tr>
				...
				<?				
				$con=dbconnecti();
				$query2=mysqli_query($con,"SELECT * FROM Patrouille WHERE Lieu='$ID' AND DATEDIFF(DATE(`Date`),'$Dateref')=-'$Datediff'  ORDER BY ID DESC, Joueur ASC");
				mysqli_close($con);
				if($query2)
				{
					while($data2=mysqli_fetch_array($query2))
		...
		<table class='table table-striped'>
			<thead><tr>
				...
				<?
				$con=dbconnecti();
				$query1=mysqli_query($con,"SELECT DISTINCT * FROM DCA WHERE Lieu='$ID' AND DATEDIFF(DATE(`Date`),'$Dateref')=-'$Datediff' ORDER BY ID DESC, Joueur ASC");
				mysqli_close($con);
				if($query1)
				{
					while($data1=mysqli_fetch_assoc($query1))
		...
		<table class='table table-striped'>
			<thead><tr>
				...
				<?
				$con=dbconnecti();
				$query2=mysqli_query($con,"SELECT DISTINCT * FROM Chasse WHERE Lieu='$ID' AND DATEDIFF(DATE(`Date`),'$Dateref')=-'$Datediff' ORDER BY ID DESC");
				mysqli_close($con);
				if($query2)
				{
					while($data2=mysqli_fetch_assoc($query2))
			...
			<div id="tab_ground">
			<table class='table table-striped'>
				<thead><tr><th colspan="9">Pertes Terrestre</th></tr></thead>
			<?
				$Veh_Axe="<table>";
				$Veh_Allie="<table>";
				$con=dbconnecti(4);
				$Veh_Atk=mysqli_query($con,"SELECT COUNT(Avion_Nbr),Unit,Avion,Date FROM Events_Ground_Stats WHERE Event_Type IN (400,401,405,420,605) AND Lieu='$ID' AND DATEDIFF(DATE(Date),'$Dateref') = -'$Datediff' GROUP BY Avion");
				//$Veh_Bomb = mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Events_Ground_Stats WHERE Lieu='$ID' AND Event_Type IN (405) AND DATEDIFF(DATE(Date),'$Dateref') = -'$Datediff'"),0);
				//$Veh_Mines = mysqli_result(mysqli_query($con,"SELECT COUNT(*) FROM Events_Ground_Stats WHERE Lieu='$ID' AND Event_Type IN (420) AND DATEDIFF(DATE(Date),'$Dateref') = -'$Datediff'"),0);
				mysqli_close($con);
				if($Veh_Atk)
				{
				...
				echo "<tr><td>".$Veh_Axe.$Veh_France.$Veh_Ita."</table></td><td>".$Veh_Allie."</table></td></tr>";
				echo "</table></div>";
				echo "<a href='index.php?view=missions_cr' class='btn btn-default' title='Retour'>Retour</a>";
			}
		}
		else
