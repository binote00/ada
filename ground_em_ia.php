...
if($OfficierID >0 xor $OfficierEMID >0)
{
    ...
    elseif($OfficierEMID >0)
    {
        $con=dbconnecti();
        $Admin=mysqli_result(mysqli_query($con,"SELECT Admin FROM Joueur WHERE ID='".$_SESSION['AccountID']."'"),0);
        $resulto=mysqli_query($con,"SELECT Front,Pays,Armee FROM Officier_em WHERE ID = $OfficierEMID");
        if($resulto)
        {
            while($datao=mysqli_fetch_array($resulto,MYSQLI_ASSOC))
            {
...
//                                if($CT_Spec_Blitz <1)$CT_Spec_Blitz=1;
            ...
            $mes.=$menu_cat_list;
            //Hangar & Garage
            if($mobile != MOBILE_RAIL and $mobile != MOBILE_WATER and $Veh_Type != TYPE_TRUCK and !$Move)
            {
                if($Arme_AT or $Arme_Art or $Categorie == CAT_DCA)
                {
                    $title_conso_atk='Consommation attaque';
                    if($Categorie == CAT_DCA and $Arme_AA)
                    {
                        //$Stock_AA;
                ...
                if($Credits >0 and $ValeurStrat >=2 and $Industrie and $Placement == PLACE_USINE and $Faction ==$Faction_Flag)
                {
                    if(!$Faction_Usine)
