...
if(isset($_SESSION['AccountID']) AND $Pilote_pvp >0 AND $avion >0 AND !empty($_POST))
{
	...
	if($Chk_tirer)
	{
		...
		mail("binote@hotmail.com","Aube des Aigles: Init Mission F5 (shoot) : ".$Pilote_pvp , "Joueur ".$Pilote_pvp." (IP ".$_SERVER['REMOTE_ADDR'].") depuis la page ".$_SERVER['HTTP_REFERER']." a tenté de charger la page ".$_SERVER['REQUEST_URI']." en utilisant ".$_SERVER['HTTP_USER_AGENT']);
	}	
	$con=dbconnecti();
	$result=mysqli_query($con,"SELECT S_HP,Ailier,S_Ailier,Equipage,Pilotage,Tactique,Tir,Courage,Reputation,S_Avion_Bombe,S_Avion_Bombe_Nbr,Simu,S_Essence,S_Baby,S_Blindage,S_Formation,
	S_Avion_db,S_Nuit,S_Cible,S_Mission,S_Engine_Nbr,S_Longitude,S_Latitude,S_Avion_Mun,S_Equipage_Nbr,S_Engine_Nbr_Eni,S_Leader,Degats_Max,
	Slot1,Slot4,Slot6,Slot7,Slot9,Slot11,Sandbox FROM Pilote_PVP WHERE ID='$Pilote_pvp'") or die('Le jeu a rencontré une erreur, merci de le signaler sur le forum avec la référence suivante : shoot_pvp-player');
	mysqli_close($con);
	if($result)
	{
	...
	else
	{
		...
		$result=mysqli_query($con,"SELECT Pays,Puissance,Robustesse,Masse,ArmePrincipale,Arme1_Nbr,ArmeSecondaire,Arme2_Nbr,Blindage,Type,Radio,Autonomie,Baby,ManoeuvreB,ManoeuvreH,Maniabilite FROM $Avion_db WHERE ID='$avion'")
		 or die('Le jeu a rencontré une erreur, merci de le signaler sur le forum avec la référence suivante : shoot_pvp-avion');
		$result2=mysqli_query($con,"SELECT Nom,Pays,Type,Robustesse,ArmePrincipale,ArmeSecondaire,ArmeArriere,TourelleSup,TourelleVentre,Blindage,ManoeuvreB,ManoeuvreH,Maniabilite FROM $Avion_db_eni WHERE ID='$avion_eni'")
		 or die('Le jeu a rencontré une erreur, merci de le signaler sur le forum avec la référence suivante : shoot_pvp-avioneni');
		//mysqli_close($con);
		if($result)
		{
			while($data=mysqli_fetch_array($result,MYSQLI_ASSOC))
		...
		switch($Action)
		{
			...
			case 2:
				...
				{					
					...
					else
					{
						...
						$titre="Combat";
						$mes.='<form action=\'index.php?view=finish_pvp\' method=\'post\'>
			...
			case 5:
				...
				elseif(IsEnrayage($Arme2Avion,$alt,$Pilote_pvp,"Stress_Arme2",true))
				{
					$intro.="<p><b>Vos armes s'enrayent au plus mauvais moment!</b></p>";
					$img="<img src='images/enrayage.jpg' style='width:100%;'>";
					$mes.=".";//"<br>Votre adversaire se replace, le combat continue!";
			...
			case 9:
				//Rompre le combat.
				/*if($HP_eni <$HPmax_eni and !$Chk_tirer and $Cible and $Pilote_eni)
