    ...
    if ($ok) {
        while ($data = mysqli_fetch_array($ok)) {
            ...
            if ($Admin and $data['Expediteur'] == 0)
                $Msg .= "<tr><td><form action='index.php?view=mail/ground_msg_envoi' method='post'><input type='hidden' name='mes' value='" . $data['ID'] . "'>" . $Lire . "<td align='left'>[ANIM] " . $Msg_Off . "</td></tr>";
            else
                $Msg .= "<tr></td><td><form action='index.php?view=mail/ground_msg_envoi' method='post'><input type='hidden' name='mes' value='" . $data['ID'] . "'>" . $Lire . "<td align='left'>" . $Msg_Off . "</td></tr>";
        }
