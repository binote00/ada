...
?>
<div><h2>Tableau des avions pilotés par des joueurs le plus souvent abattus</h2>
<table class='table'>
	<tr><thead><th>N°</th>
		<th>Nom</th>
		<th>Pays</th>
...
$i=0;
$con=dbconnecti();
$result=mysqli_query($con,"SELECT Avion_loss,COUNT(*) FROM Chasse WHERE PVP=1 GROUP BY Avion_loss ORDER BY COUNT(*) DESC");
mysqli_close($con);
if($result)
{
	while($data = mysqli_fetch_array($result,MYSQLI_NUM))
	{
			...
			if($result2)
			{
				while($avion_data=mysqli_fetch_array($result2,MYSQLI_ASSOC))
				{
					...
					$con=dbconnecti(4);
					$resultp=mysqli_query($con,"SELECT COUNT(*) FROM Events WHERE Event_Type=1 AND Avion='$ID'");
					mysqli_close($con);
					if($resultp)
					{
