...
if(isset($_SESSION['AccountID']))
{
	...
	{
		$con=dbconnecti();
		$result=mysqli_query($con,"SELECT Reputation,Credits,MIA,Pays,Slot1,Slot2,Slot3,Slot4,Slot5,Slot6,Slot7,Slot8,Slot9,Slot10,Slot11 FROM Pilote WHERE ID='$PlayerID'")
		or die('Le jeu a rencontré une erreur, merci de le signaler sur le forum avec la référence suivante : inv-ply');
		mysqli_close($con);
		if($result)
		{
		...
		if($Credits >0 and $Reput >49 and !$MIA)
		{		
			function PrintMatos($Slot)
			{
				...
				$txt='';
				$con=dbconnecti(1);
				$result=mysqli_query($con,"SELECT DISTINCT ID,Nom,Cout,Reput_mini FROM Matos WHERE Slot='$Slot' AND Reput_mini <='$Reput' AND Cout <98 AND Pays IN (0,'$country') ORDER BY Cout,Nom ASC");
				mysqli_close($con);
				if($result)
				{
					while($data=mysqli_fetch_array($result,MYSQLI_ASSOC)) 
					{
						$txt.="<option value='".$data['ID']."'>".$data['Nom']." (".$data['Cout']." CT)</option>";
					}
					mysqli_free_result($result);
				}
				unset($data);
...
<?
			...
			echo "<h1>Inventaire</h1><div class='alert alert-danger'>Vous n'avez actuellement pas le temps pour ça!</div>";
	}
	else
