...
if(isset($_SESSION['AccountID']) AND $PlayerID >0)
{
	...
	{
		$con=dbconnecti();
		$Unite=mysqli_result(mysqli_query($con,"SELECT Unit FROM Pilote WHERE ID='".$PlayerID."'"),0);
		$resultu=mysqli_query($con,"SELECT Nom,Base,Type FROM Unit WHERE ID='$Unite'");
		mysqli_close($con);
		if($resultu)
		{
		...
		if($Unite_Type ==8)
			echo '<h1>'.$Unite_Nom."</h1><div class='alert alert-info'>Lorsque vous aurez terminé votre formation et que votre demande de mutation sera validée, vous pourrez consulter le journal de votre nouvelle escadrille.</div>";
		else
		{
			if($Tab =="attaques")
			{
				$in="23,24,28";
				$in_base="76,77,142,272,273";
				$query="(SELECT ID,Event_Type,`Date`,Lieu,PlayerID,Avion,Avion_Nbr,Pilote_eni FROM Events WHERE Unit='$Unite' AND Event_Type IN (".$in.") AND `Date` > '".$Date_logs_esc."') 
				UNION ALL (SELECT ID,Event_Type,`Date`,Lieu,PlayerID,Avion,Avion_Nbr,Pilote_eni FROM Events_em WHERE Lieu='$Unite_Base' AND Event_Type IN (".$in_base.") AND `Date` > '".$Date_logs_esc."')
			...
			elseif($Tab =="missions")
			{
				...
				$query="SELECT ID,Event_Type,`Date`,Lieu,PlayerID,Avion,Avion_Nbr,Pilote_eni FROM Events_em WHERE Unit='$Unite' AND Event_Type IN (".$in_em.") AND `Date` > '".$Date_logs_esc."' ORDER BY ID DESC LIMIT 100";
			}
			elseif($Tab =="pilotes")
			{
				$in="18,19,30,31,32,33,34,36";
				$query="SELECT ID,Event_Type,`Date`,Lieu,PlayerID,Avion,Avion_Nbr,Pilote_eni FROM Events WHERE Unit='$Unite' AND Event_Type IN (".$in.") AND `Date` > '".$Date_logs_esc."' ORDER BY ID DESC LIMIT 100";
			}
			elseif($Tab =="ravit")
			{
				$in_em="101,102,103,104,105,106,107,111,112,113,114,137,139,141"; //25
				$in_base="72,73,272,273";
				$query="(SELECT ID,Event_Type,`Date`,Lieu,PlayerID,Avion,Avion_Nbr,Pilote_eni FROM Events_ravit WHERE Unit='$Unite' AND Event_Type IN (".$in_em.") AND `Date` > '".$Date_logs_esc."') 
				UNION ALL (SELECT ID,Event_Type,`Date`,Lieu,PlayerID,Avion,Avion_Nbr,Pilote_eni FROM Events_ravit WHERE Lieu='$Unite_Base' AND Event_Type IN (".$in_base.") AND `Date` > '".$Date_logs_esc."') ORDER BY ID DESC LIMIT 100";
			}
			else
			{
				$in="18,19,23,24,28,41";
				$in_base="76,77,142,272,273";
				$query="(SELECT ID,Event_Type,`Date`,Lieu,PlayerID,Avion,Avion_Nbr,Pilote_eni FROM Events WHERE Unit='$Unite' AND Event_Type IN (".$in.") AND `Date` > '".$Date_logs_esc."') 
				UNION ALL (SELECT ID,Event_Type,`Date`,Lieu,PlayerID,Avion,Avion_Nbr,Pilote_eni FROM Events_em WHERE Lieu='$Unite_Base' AND Event_Type IN (".$in_base.") AND `Date` > '".$Date_logs_esc."')
				UNION ALL (SELECT ID,Event_Type,`Date`,Lieu,PlayerID,Avion,Avion_Nbr,Pilote_eni FROM Events_em WHERE Pilote_eni='$Unite' AND Event_Type IN(222,223,224)) ORDER BY ID DESC LIMIT 100";
			...
			if($result)
			{
				while($Classement=mysqli_fetch_array($result,MYSQLI_ASSOC)) 
				{
					...
					switch($Event_Type)
					{
						...
						break;
						case 30:
							$Pays_medal=GetData("Pilote","ID",$Event_PlayerID,"Pays");
						...
						break;
						case 77:
							$Event.=$Event_Date." : La batterie de DCA <b>".GetData("Armes","ID",$Event_Pilote_eni,"Nom")."</b> a endommagé un ".$Event_Avion_Nom." au-dessus de notre base de ".$Event_Lieu_Nom."<br>";
						break;
						case 79:
							$Event.=$Event_Date." : La batterie de DCA <b>".GetData("Armes","ID",$Event_Pilote_eni,"Nom")."</b> a endommagé un ".$Event_Avion_Nom." au-dessus de notre base de ".$Event_Lieu_Nom."<br>";
						break;
						...
						case 96:
							...
							$Event.=$Event_Date." : ".$Pilote_eni." escortant ".$Event_Pilote_Nom." a été abattu par un ".$Event_Avion_eni_Nom." dans les environs de ".$Event_Lieu_Nom.". Un <b>".$Event_Avion_Nom."</b> a été perdu !</b>.<br>";
						break;
						case 97:
						...
						case 101:
							if($Event_Avion ==1)
								...
								$Octane=$Event_Avion." Octane</b>";
							$Event.=$Event_Date." : ".$Event_Pilote_Nom." a commandé <b>".$Event_Avion_Nbr." litres de ".$Octane.", arrivés sur notre base de ".$Event_Lieu_Nom."<br>";
						break;
						case 102:
						...
						break;			
						case 107:
							$Event.=$Event_Date." : ".$Event_Pilote_Nom." a camouflé notre base de ".$Event_Lieu_Nom."<br>";
						break;			
						case 111:
						...
						break;
						case 179:
							$Event.=$Event_Date." : Une batterie de DCA ennemie a endommagé le ".$Event_Avion_Nom." de <b>".GetData("Armes","ID",$Event_Pilote_eni,"Nom")."</b> au-dessus de ".$Event_Lieu_Nom."<br>";
						break;
						case 180:
							if($Event_Pilote_eni)
							{
			...
			echo "<div style='overflow:auto; height: 640px;'><table class='table'><tr><td rowspan='20' align='left'>".$Event."</td></tr></table></div>".$msc;
		}
		if($msc >5)
		{
			mail('binote@hotmail.com','Aube des Aigles: Slow EscJournal',$msc.' secondes pour unité '.$Unite_Nom.' ('.$Unite.')');
