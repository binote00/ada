                        ...
                        if ($pj_unit) {
                            ...
                            while ($data = mysqli_fetch_array($pj_unit)) {
                                if ($data['ID'] > 0) {
                                    ...
                                    if ($Taille >= 2) {
                                        $Shoot = mt_rand(0, 50) + ($Stab / 10) - $Malus_Reperer_reg + $Taille + ($meteo * 3) - ($Mission_alt / 10);
                                        $Photo_shoot = mt_rand(0, 50) + $Bonus_Camera + ($Stab / 10) + ($meteo * 2) - ($Mission_alt / 100) - $Malus_Reperer_reg;
                                        if ($Admin) $debug_intro .= "<br>Shoot=" . $Shoot . " (+Vue_ailier=0/50, -meteo*3=" . $meteo . ", +Taille=" . $Taille . ", -Malus_Reperer_reg=" . $Malus_Reperer_reg .
                ...
                if ($Credits >= 4) {
                    ...
                    if ($Patrol_Nbr > 0)
                        ...
                        if ($Avion_Nbr > 0) {
                            ...
                            if ($pj_unit) {
                                while ($data = mysqli_fetch_array($pj_unit)) {
                                    if (mt_rand(0, 50) + $Radar_Bonus + $Meteo + $data['Taille'] - mt_rand(0, $data['Experience']) > 0)
                                        $Detect_asm = true;
                                    else
                                        $Detect_asm = false;
                                    if (($data['Visible'] > 0 or $Detect_asm) and $data['ID'] > 0 and $Avions_Bomb > 0) {
                                        ...
                                        $rand_tir = mt_rand(0, 25);
                                        $Shoot = $rand_tir + ($Stab / 10) + $meteo - $Esquive - ($Mission_alt / 100);
