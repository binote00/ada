...
{	
	...
	if($Credits >=$CT)
	{
		...
		if($Vehicule_Nbr >0)
		{
			...
			if($result)
			{
				...
				if($Update_xp >0 and $Reco_by_Fire)
					mail("binote@hotmail.com","Aube des Aigles: Reco By Fire","Joueur : ".$OfficierID." dans les environs de : ".$Cible_nom."<br>Reco de ".$Veh_Nom." <html>".$intro.$mes."</html>","Content-type: text/html; charset=utf-8");
			}
			$skills.="</div>";
		...
		//mail("binote@hotmail.com", "Aube des Aigles: Combat : Reco", "Joueur : ".$OfficierID." dans les environs de : ".$Cible_nom."<br>Reco de ".$Veh_Nom."<html>".$intro.$mes.$skills."</html>", "Content-type: text/html; charset=iso-8859-1");
		if($alerte_reco)
		{
