...
if (isset($_SESSION['AccountID'])) {
    ...
    if ($OfficierEMID > 0) {
        ...
        if ($Unite) {
            ...
            if ($Porte_avions > 5000) {
                dbconnect();
                $result = $dbh->prepare("SELECT a.ID FROM Armee a
                LEFT JOIN Division d ON d.Armee = a.ID
                LEFT JOIN Regiment_IA ri ON ri.Division = d.ID
                LEFT JOIN Cible c ON c.ID = ri.Vehicule_ID
                LEFT JOIN Unit u ON u.Porte_avions = c.ID
                WHERE u.Porte_avions = :pa");
                $result->bindParam('pa', $Porte_avions, 1);
                $result->execute();
                $Unit_Armee = $result->fetchObject()->ID;
            }
        }
        if ($Credits > 0 and $Base > 0 and !$Mission_IA) {
            ...
            if ($OfficierEMID == $Commandant or $OfficierEMID == $Officier_Adjoint or ($OfficierEMID == $Officier_Mer and $Porte_avions) or $Off_EM_Sub or $GHQ or $Admin or ($Armee > 0 and ($Unit_Armee == $Armee))) {
                ...
                if ($OfficierEMID == $Commandant or $OfficierEMID == $Officier_Adjoint or $GHQ or $Admin) //Menu Unites
                {
                    $menu_mission = "<p><span class='btn btn-info'>Unités ></span> ";
                    if ($Unite_Type == AVION_ATTAQUE)
                    ...
                    if ($Unite_Type == AVION_BOMBARDIER)
                    ...
                    if ($Unite_Type == AVION_CHASSE_EMB)
                ...
                if (!$Off_EM_Sub and !$Cdt) {
                    ...
                    } elseif (($OfficierEMID == $Commandant or $Admin) and ($Unite_Type == AVION_CHASSE or $Unite_Type == AVION_BOMBARDIER or $Unite_Type == AVION_RECO or $Unite_Type == AVION_TRANSPORT or $Unite_Type == AVION_ATTAQUE or $Unite_Type == AVION_PAT_MAR)) {
                        if ($Unite_Type == AVION_PAT_MAR)
                            $query_add_armees = " AND Maritime=1";
                        else
                            $query_add_armees = '';
                        $con = dbconnecti();
                        $res_armees = mysqli_query($con, "SELECT ID,Nom FROM Armee WHERE Pays='$country' AND Front='$Front_unit' AND Active=1 AND Cdt >0" . $query_add_armees);
                        mysqli_close($con);
                        if ($res_armees) {
                ...
                {
                    $help_txt_autonomie = 'Distance maximale de l\'objectif pour cette mission.<<br>Cette valeur ne peut être supérieure à l\'autonomie la plus faible de chaque modèle d\'avion de tous les ' . $Sqn . ' de l\'unité.<br>Cette valeur est augmentée de la valeur moyenne de la compétence navigation des pilotes de l\'unité.';
                    if ($Porte_avions > 0) {
                    ...
                    $today = getdate();
                    if ($Credits < 1)
                        echo "<div class='alert alert-danger'>Vous ne disposez pas de suffisamment de Crédits Temps pour assigner une mission à votre unité !</div>";
                    elseif (($Jour_m == $today['mday'] and $today['mon'] == $Mois_m) or ($today['hours'] > 1 and $today['hours'] < 7))
                    ...
                    elseif ($QualitePiste > 50 and (($Avion1_Nbr + $Avion2_Nbr + $Avion3_Nbr) > 0) and $Faction == $Faction_Flag and $Faction == $Faction_Air and $Meteo > -50 and !$Mission_IA and $Pilotes > 0) {
                        if (($Unite_Type == AVION_BOMBARDIER or $Unite_Type == AVION_TRANSPORT or $Unite_Type == AVION_PAT_MAR or $Unite_Type == AVION_BOMB_LD) and $Meteo < -19 and $BaseAerienne > 2 and !$Hydravion)
                            ...
                            if (!$Porte_avions and !$Nation_IA) {
                                ...
                                $Stock_100_max = 0;
                                $Stock_1_max = 0;
                                $Auto_Log = GetAutoLog($Front_unit, false, false, true);
                                $con = dbconnecti();
                            ...
                            $Lat_base_min = $Coord[0] - 2;
                            $Lat_base_max = $Coord[1] + 2;
                            $Long_base_min = $Coord[2] - 1;
                            $Long_base_max = $Coord[3] + 1;
                            if ($Front_unit == 0) {
                            ...
                            if ($Flight_txt and !$Canada) {
                                if (!$Admin) {
                                    if ($G_Treve or ($G_Treve_Med and $Front == FRONT_MED) or ($G_Treve_Est_Pac and ($Front == FRONT_EST or $Front == FRONT_NORD or $Front == FRONT_PAC))) $query_treve = " AND Flag='$country'";
                                }
                                if ($Unite_Type == AVION_TRANSPORT and $choix15) {
                                    if ($Only_Here or $G_Treve or ($G_Treve_Med and $Front == 2) or ($G_Treve_Est_Pac and ($Front == 1 or $Front == 4 or $Front == 3)))
                                        $Lieuxo .= "<option value=" . $Base . ">" . $Base_Nom . " (10km)</option>";
                                    else {
                                        $con = dbconnecti();
                                        $Paras_res = mysqli_query($con, "SELECT ID,Position FROM Regiment_IA WHERE Lieu_ID='$Base' AND Pays='$country' AND Placement=1 AND Position IN(12,13)");
                                        mysqli_close($con);
                                        if ($Paras_res) {
                                        ...
                                        if ($Lieuxo) {
                                            $Strat_Output_txt .= "<div class='panel panel-war'><div class='panel-heading'>Ravitaillement</div><div class='panel-body'><a href='#' class='popup'><b class='label label-warning'>Rayon d'action " . $Autonomie_max . "km</b><span>" . $help_txt_autonomie . "</span></a>
                                        ...
                                        if ($Paras_units or $Cdo_units) {
                                            ...
                                            if ($Paras_units) {
                                                $Strat_Output_txt .= "<div class='panel panel-war'><div class='panel-heading'>Parachutage</div><div class='panel-body'><a href='#' class='popup'><b class='label label-warning'>Rayon d'action " . $Autonomie_max . "km</b><span>" . $help_txt_autonomie . "</span></a>
                                                <form action='em_ia1.php' method='post'>
                                                <input type='hidden' name='Unite' value='" . $Unite . "'>
                                                <input type='hidden' name='Avion1' value='" . $Avion1 . "'>
                                                <input type='hidden' name='Avion2' value='" . $Avion2 . "'>
                                            ...
                                            if ($Cdo_units) {
                                                $Strat_Output_txt .= "<div class='panel panel-war'><div class='panel-heading'>Commando</div><div class='panel-body'><a href='#' class='popup'><b class='label label-warning'>Rayon d'action " . $Autonomie_max . "km</b><span>" . $help_txt_autonomie . "</span></a>
                                                <form action='em_ia1.php' method='post'>
                                                <input type='hidden' name='Unite' value='" . $Unite . "'>
                                                <input type='hidden' name='Avion1' value='" . $Avion1 . "'>
                                                <input type='hidden' name='Avion2' value='" . $Avion2 . "'>
                                ...
                                elseif ($Unite_Type == AVION_RECO and $choix15) {
                                    $Lieuxo = '';
                                    if ($Only_Here or $G_Treve or ($G_Treve_Med and $Front == FRONT_MED) or ($G_Treve_Est_Pac and ($Front == FRONT_EST or $Front == FRONT_NORD or $Front == FRONT_PAC)))
                                        $Lieuxo .= "<option value=" . $Base . ">" . $Base_Nom . " (10km)</option>";
                                    else {
                                        $query = "SELECT DISTINCT ID,Nom,Longitude,Latitude FROM Lieu WHERE Latitude>='$Lat_base_min' AND Latitude<='$Lat_base_max' AND Longitude>='$Long_base_min' AND Longitude<'$Long_base_max' AND Flag<>'$country' AND Zone<>6 ORDER BY Nom ASC";
                                        $con = dbconnecti();
                                    ...
                                    if ($Lieuxo) {
                                        ...
                                        if ($Plafond_max >= 10000)
                                            $select_alt .= "<option value='10000'>Haute altitude (10000m)</option>";
                                        $Strat_Output_txt .= "<div class='panel panel-war'><div class='panel-heading'>Mission Stratégique<a href='#' class='popup'><b class='label label-primary'>Rayon d'action " . $Autonomie_max . "km</b><span>" . $help_txt_autonomie . "</span></a></div><div class='panel-body'>
											<form action='em_ia1.php' method='post'>
											<input type='hidden' name='Unite' value='" . $Unite . "'>
											<input type='hidden' name='Avion1' value='" . $Avion1 . "'>
											<input type='hidden' name='Avion2' value='" . $Avion2 . "'>
                                ...
                                elseif ($Unite_Type == AVION_BOMBARDIER or $Unite_Type == AVION_ATTAQUE or $Unite_Type == AVION_BOMB_LD) {
                                    $Lieuxo = '';
                                ...
                                if ($Unite_Type == AVION_PAT_MAR and $Credits >= 4) //Pat Mar
                                {
                                    $Lieuxasm = '';
                                    $Lieuxt2 = '';
                                    $query = "SELECT DISTINCT ID,Nom,Longitude,Latitude,Zone FROM Lieu WHERE Latitude >='$Lat_base_min' AND Latitude <='$Lat_base_max' AND Longitude >='$Long_base_min' AND Longitude <'$Long_base_max' AND (Zone=6 OR Port_Ori >0 OR Plage >0)" . $query_treve . " ORDER BY Nom ASC";
                                    $con = dbconnecti();
                                    $result = mysqli_query($con, $query) or die(mysqli_error($con));
                                    if ($result) {
                                        while ($data = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                            ...
                                            if ($Dist[0] <= $Autonomie_max) {
                                                ...
                                                if ($data['Zone'] == 6 and $Admin) {
                                                    $result_unit2 = false;
                                                    $query_unit2 = "SELECT COUNT(*) FROM Regiment_IA as r,Pays as p WHERE r.Pays=p.ID AND r.Lieu_ID='" . $data['ID'] . "' AND r.Vehicule_Nbr >'0' AND p.Faction<>'" . $Faction . "' AND r.Visible='1' AND r.Bomb_IA='0'";
                                                    $result_unit2 = mysqli_result(mysqli_query($con, $query_unit2), 0);
                                                    if ($result_unit2 > 0) {
                                                        if ($Admin)
                                                            $ad_txt = '<b>- ' . $result_unit2 . ' cibles</b>';
                                                        else
                                                            $ad_txt = '';
                                                        $Lieuxt2 .= "<option value='" . $data['ID'] . "'>" . $data['Nom'] . " (" . $Dist[0] . "km)" . $ad_txt . "</option>";
                                                    }
                                                }
                                            }
                                        }
                                    ...
                                    mysqli_close($con);
                                    if ($Lieuxt2) {
                                        $Tac_Output_txt .= "<div class='panel panel-war'><div class='panel-heading'>Bombardement Naval<a href='#' class='popup'><b class='label label-primary'>Rayon d'action " . $Autonomie_max . "km</b><span>" . $help_txt_autonomie . "</span></a></div><div class='panel-body'>
												<form action='index.php?view=em_ia5' method='post'>
												<input type='hidden' name='Unite' value='" . $Unite . "'>
												<div class='row'>
												    <div class='col-xs-12 col-sm-8'>
												        <label for='Cible'>Choix de la Cible</label>
												        <select name='Cible' class='form-control'>" . $Lieuxt2 . "</select>
												    </div>
												    <div class='col-xs-12 col-sm-4'>
												        <label for='Flight'>Choix du " . $Sqn . "</label>
												        <select name='Flight' class='form-control'>" . $Flight_txt . "</select>
                                    ...
                                    if ($Lieuxasm) {
                                        $Tac_Output_txt .= "<div class='panel panel-war'><div class='panel-heading'>ASM<a href='#' class='popup'><b class='label label-primary'>Rayon d'action " . $Autonomie_max . "km</b><span>" . $help_txt_autonomie . "</span></a></div><div class='panel-body'>
											<form action='em_ia1.php' method='post'>
											<input type='hidden' name='Unite' value='" . $Unite . "'>
											<input type='hidden' name='Avion1' value='" . $Avion1 . "'>
											<input type='hidden' name='Avion2' value='" . $Avion2 . "'>
                                ...
                                if ($Unite_Type < 13 and $Unite_Type != 8 and ($choix17 or $choix4 or $choix5 or $choix7 or $choix32)) {
                                    ...
                                    if ($Only_Here)
                                        ...
                                        elseif ($Admin) {
                                            ...
                                            FROM Lieu WHERE Latitude >='$Lat_base_min' AND Latitude <='$Lat_base_max' AND Longitude >='$Long_base_min' AND Longitude <'$Long_base_max' 
                                    ...
                                    if ($Lieuxd and $choix5) {
                                        if ($Unite_Type == AVION_CHASSE or $Unite_Type == AVION_BOMBARDIER or $Unite_Type == AVION_CHASSE_LD or $Unite_Type == AVION_ATTAQUE or $Unite_Type == AVION_CHASSE_EMB) {
                                        ...
                                        if ($Flight_txt_rec) {
                                            $Tac_Output_txt .= "<div class='panel panel-war'><div class='panel-heading'>Reconnaissance " . $Aide_txt_tac . "<a href='#' class='popup'><b class='label label-primary'>Rayon d'action " . $Autonomie_max . "km</b><span>" . $help_txt_autonomie . "</span></a></div><div class='panel-body'>
												<form action='em_ia1.php' method='post'>
												<input type='hidden' name='Unite' value='" . $Unite . "'>
												<input type='hidden' name='Avion1' value='" . $Avion1 . "'>
												<input type='hidden' name='Avion2' value='" . $Avion2 . "'>
                                    ...
                                    if ($Lieuxd and ($choix17 or $choix4 or $choix7 or $choix32)) {
                                        ...
                                        if ($Plafond_max >= 10000)
                                            $select_alt .= "<option value='10000'>Haute altitude (10000m)</option>";
                                        if ($Unite_Type == AVION_CHASSE or $Unite_Type == AVION_CHASSE_LD or $Unite_Type == AVION_CHASSE_EMB)
                                            $Mis_Chs_txt = 'Chasse';
                                        else
                                            $Mis_Chs_txt = "Veille <a href='help/aide_task.php' target='_blank' title='Cliquez pour aide'><img src='images/help.png'></a>";
                                        $Tac_Output_txt .= "<div class='panel panel-war'><div class='panel-heading'>" . $Mis_Chs_txt . "<a href='#' class='popup'><b class='label label-primary'>Rayon d'action " . $Autonomie_max . "km</b><span>" . $help_txt_autonomie . "</span></a></div><div class='panel-body'>
											<form action='em_ia1.php' method='post'>
											<input type='hidden' name='Unite' value='" . $Unite . "'>
											<input type='hidden' name='Avion1' value='" . $Avion1 . "'>
											<input type='hidden' name='Avion2' value='" . $Avion2 . "'>
                                            ...
                                            <div class='row'>
                                                ...
                                                <div class='col-xs-12 col-sm-6'>
                                                    <label for='Type'>Choix du Type de Mission</label>
                                                    <select name='Type' class='form-control'>" . $choix17 . $choix7 . $choix4 . $choix32 . "</select>
                                                </div>
                                            </div>
                                            <div class='row'>
                                                <div class='col-xs-12 col-sm-8'>
                                                    ...
                                                    <option value='1000'>Basse altitude (1000m)</option>
                                                    <option value='2000'>Altitude moyenne (2000m)</option>
                                                    <option value='3000' selected>Altitude moyenne (3000m)</option>" . $select_alt . "</select>
                                                </div>
                                                <div class='col-xs-12 col-sm-4'>
                                                    <label for='Flight'>Choix du " . $Sqn . "</label>
                                                    <select name='Flight' class='form-control'>" . $Flight_txt . "</select>
                                        ...
                                        elseif ($Unite_Type == AVION_CHASSE_LD)
                                    ...
                                    if ($Lieuxlong and ($choix17 or $choix4 or $choix7) and $Credits >= 2) {
                                        ...
                                        $Tac_Output_txt .= "<div class='panel panel-war'><div class='panel-heading'>Longue distance<span class='label label-warning'>Rayon d'action " . $Autonomie_long_max . "km</span></div><div class='panel-body'>                                        
                                            ...
                                            <div class='row'>
                                                ...
                                                <div class='col-xs-12 col-sm-6'>
                                                    <label for='Type'>Choix du Type de Mission</label>
                                                    <select name='Type' class='form-control'>" . $choix17 . $choix7 . $choix4 . "</select>
                                                </div>
                                            </div>
                                            <div class='row'>
                                                <div class='col-xs-12 col-sm-8'>
