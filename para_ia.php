...
if($PlayerID >0 xor $Officier >0 xor $OfficierEMID >0)
{
	...
	$query="SELECT a.*,u.Nom as Unite_s,u.Pays,l.Nom FROM Attaque_ia as a,Unit as u,Lieu as l WHERE a.Unite=u.ID AND a.Lieu=l.ID AND l.Latitude >='$Lat_base_min' AND l.Latitude <='$Lat_base_max' AND l.Longitude >='$Long_base_min' AND l.Longitude <'$Long_base_max' AND a.Arme=0 ORDER BY a.ID DESC LIMIT 25";
	$liste_ia="";
	...
	if($result)
	{
		...
		if($liste_ia)
		{
			echo "<h1>Parachutages EM</h1><a href='index.php?view=paras' class='btn btn-primary'>Parachutages Joueurs</a><div style='overflow:auto; width: 100%;'><table class='table table-striped'>
