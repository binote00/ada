...
if(isset($_SESSION['AccountID']) and $PlayerID >0 AND !empty($_POST))
{
	...
	$con=dbconnecti();
	$reset=mysqli_query($con,"UPDATE Pilote_PVP SET S_Strike=0,S_Cible=0,S_Escorte=0,S_Escorteb=0,S_Escorte_nbr=0,S_Escorteb_nbr=0,S_Equipage_Nbr=1,S_Leader=0,S_Ailier=0,S_Intercept_nbr=0,enis=0,avion_eni=0,Sandbox=0 WHERE ID='$PlayerID'") 
	or die('Le jeu a rencontré une erreur, merci de le signaler sur le forum avec la référence suivante : mission1_pvp-reset');
	$result=mysqli_query($con,"SELECT Unit,Front,Pilotage,Courage,Moral,Endurance,Crashs_Jour,Avancement,Missions_Max,Proto,Avion_Perso,S_Blindage,Heure_Mission FROM Pilote_PVP WHERE ID='$PlayerID'") 
	or die('Le jeu a rencontré une erreur, merci de le signaler sur le forum avec la référence suivante : mission1_pvp-player');
	mysqli_close($con);
	if($result)
	{
	...
	{		
		...
		if(($Grade[1] <2 and $Unite_Type !=8) or ($Unite_Type ==8 and $Pilotage <20))
		{
			if($AvionT1_Nbr >0) //and $Avion1_Limite >= $Crashs_Jour and GetPil($PlayerID,$AvionT1,1) >=$Avion1_XP)
				SetData("Pilote","S_Leader",1,"ID",$PlayerID);
			else
			{
				$AvionT1_Nbr=0;
				if($Crashs_Jour > $Avion1_Limite)
				{
		...
		else
		{
			...
			if($Avion_p >0)
			{
				...
				if($Avion_perso and $Bomb5 and $Unit5_Stock_Fuel > $Avion5_a)
					...
					$AvionT5_txt="<Input type='Radio' name='Avion' value='".$Avion_p."' title='Avion non disponible par manque de carburant ou de munitions, ou parce que son rôle est différent de celui de votre unité' disabled>-".GetAvionIcon($ID_ref,$country,0,$Unite,$Front)." Avion perso";
			}			
			//Avion2
			if($AvionT2_Nbr >0) //and $Avion2_Limite >= $Crashs_Jour and GetPil($PlayerID, $AvionT2, 1) >= $Avion2_XP)
			{
				...
				{		
					...
					if($Avion2_Bombe_Nbr >0 and $Avion2_Bombe and $Unite_Type !=6)
					{
						...
						$Avion2_mn=GetMani("Avion",$AvionT2,$Avion2_r,$moda);
			...
			if($AvionT3_Nbr >0) //and $Avion3_Limite >=$Crashs_Jour and GetPil($PlayerID,$AvionT3,1) >=$Avion3_XP)
			{
				...
				{		
					...
					if($Avion3_Bombe_Nbr >0 and $Avion3_Bombe and $Unite_Type !=6)
					{
						...
						$Avion3_mn=GetMani("Avion",$AvionT3,$Avion3_r,$moda);
			...
			if($AvionT1_Nbr and $Unit1_Stock_Fuel >$Avion1_a and $Unit1_stock_mun >$Avion1_mun and $Unit1_stock_mun_as >$Avion1_mun_as and $Bomb1)
			{
				...
				{*/
					if($Avion1_Bombe_nbr >0 and $Avion1_Bombe and $Unite_Type !=6)
					{
						...
						$Avion1_mn=GetMani($Avion_db,$AvionT1,$Avion1_r,$moda);
		...
		if($AvionT1_Nbr >0 or $AvionT2_Nbr >0 or $AvionT3_Nbr >0)
		{
			...
			elseif(($Type_Mission >40 and $Type_Mission <50) or ($Type_Mission >400 and $Type_Mission <440))
			{
				...
				if($Second_off >0)
					$Lieux_exclus[]=GetData("Officier","ID",$Second_off,"Mission_Lieu_D");*/
				if($Officier >0)
					$Lieu_Reg_Off=GetData("Regiment","Officier_ID",$Officier,"Lieu_ID");
				if($Second_Off >0)
					$Lieu_Reg_Off.=",".GetData("Regiment","Officier_ID",$Second_Off,"Lieu_ID");
				...
				if($Type_Mission ==7 or $Type_Mission ==17)
				{
					...
					WHERE r.Mission_Lieu_D=l.ID AND r.Pays=p.ID AND r.Mission_Type_D='$Type_Mission' AND p.Faction='$Faction' AND r.Front='$Front' AND r.ID NOT IN ('$Officier','$Second_Off')
					UNION SELECT l.ID,l.Nom,l.Latitude,l.Longitude FROM Lieu as l,Pays as p,Regiment_IA as r
					WHERE r.Mission_Lieu_D=l.ID AND r.Pays=p.ID AND r.Mission_Type_D='$Type_Mission' AND p.Faction='$Faction' AND (l.Latitude BETWEEN '$Lat_base_min' AND '$Lat_base_max') AND (l.Longitude BETWEEN '$Long_base_min' AND '$Long_base_max') AND l.ID NOT IN ('$Lieu_Reg_Off')
					UNION SELECT l.ID,l.Nom,l.Latitude,l.Longitude FROM Lieu as l,Pilote as j,Unit as u,Pays as p
					WHERE j.Unit=u.ID AND p.ID=u.Pays AND u.Mission_Lieu_D=l.ID AND u.Mission_Type_D='$Type_Mission' AND p.Faction='$Faction' AND j.Front='$Front' AND u.ID<>'$Unite' 
				...
				{
					$query="SELECT l.ID,l.Nom,l.Latitude,l.Longitude FROM Lieu as l,Pays as p,Officier as r 
					WHERE r.Mission_Lieu_D=l.ID AND r.Pays=p.ID AND r.Mission_Type_D='$Type_Mission' AND p.Faction='$Faction' AND r.Front='$Front' AND l.Zone<>6 AND r.ID NOT IN ('$Officier','$Second_Off')
					UNION SELECT l.ID,l.Nom,l.Latitude,l.Longitude FROM Lieu as l,Pays as p,Regiment_IA as r
					WHERE r.Mission_Lieu_D=l.ID AND r.Pays=p.ID AND r.Mission_Type_D='$Type_Mission' AND p.Faction='$Faction' AND (l.Latitude BETWEEN '$Lat_base_min' AND '$Lat_base_max') AND (l.Longitude BETWEEN '$Long_base_min' AND '$Long_base_max') AND l.ID NOT IN ('$Lieu_Reg_Off')
					UNION SELECT l.ID,l.Nom,l.Latitude,l.Longitude FROM Lieu as l,Pilote as j,Unit as u,Pays as p 
					WHERE j.Unit=u.ID AND p.ID=u.Pays AND u.Mission_Lieu_D=l.ID AND u.Mission_Type_D='$Type_Mission' AND p.Faction='$Faction' AND j.Front='$Front' AND l.Zone<>6 AND u.ID<>'$Unite'
					ORDER BY Nom ASC";
				}
			...
			elseif($Type_Mission ==5 and ($Unite_Type ==3 or $Unite_Type ==9))
			{
				...
				if($result)
				{
					while($data=mysqli_fetch_array($result,MYSQLI_ASSOC)) 
					{
						$Dist=GetDistance(0,0,$Long_base,$Lat_base,$data['Longitude'],$data['Latitude']);
						$Dist_chk=($Dist[0]*2)+200;
						if(($Dist_chk <$Avion1_a or $Dist_chk <$Avion2_a or $Dist_chk <$Avion3_a or $Dist_chk <$Avion4_a or $Dist_chk <$Avion5_a) and $Dist[0] >10)
						{
							$idata=$data['Nom'].' ('.$Dist[0].'km)';
							$Lieux.='<option value='.$data['ID'].'>'.$idata.'</option>';
						}
			...
			{				
				...
				{
					if($Front ==3)
						$query="SELECT DISTINCT ID,Nom,Longitude,Latitude FROM Lieu WHERE Longitude >67 AND ID<>'$Base' AND Recce_PlayerID<>'$PlayerID' AND Recce_PlayerID_TAL<>'$PlayerID' AND Recce_PlayerID_TAX<>'$PlayerID' ORDER BY Nom ASC";
					elseif($Front ==1)
						$query="SELECT DISTINCT ID,Nom,Longitude,Latitude FROM Lieu WHERE Latitude >=43 AND Latitude <50.5 AND Longitude >13 AND Longitude <50 AND ID<>'$Base' AND Recce_PlayerID<>'$PlayerID' AND Recce_PlayerID_TAL<>'$PlayerID' AND Recce_PlayerID_TAX<>'$PlayerID' ORDER BY Nom ASC";
					elseif($Front ==4)
						$query="SELECT DISTINCT ID,Nom,Longitude,Latitude FROM Lieu WHERE Latitude >50.5 AND Longitude >13 AND Longitude <50 AND ID<>'$Base' AND Recce_PlayerID<>'$PlayerID' AND Recce_PlayerID_TAL<>'$PlayerID' AND Recce_PlayerID_TAX<>'$PlayerID' ORDER BY Nom ASC";
					elseif($Front ==5)
						$query="SELECT DISTINCT ID,Nom,Longitude,Latitude FROM Lieu WHERE Latitude >60 AND Longitude >13 AND Longitude <50 AND ID<>'$Base' AND Recce_PlayerID<>'$PlayerID' AND Recce_PlayerID_TAL<>'$PlayerID' AND Recce_PlayerID_TAX<>'$PlayerID' ORDER BY Nom ASC";
					elseif($Front ==2)
						$query="SELECT DISTINCT ID,Nom,Longitude,Latitude FROM Lieu WHERE Latitude <43 AND Longitude <50 AND ID<>'$Base' AND Recce_PlayerID<>'$PlayerID' AND Recce_PlayerID_TAL<>'$PlayerID' AND Recce_PlayerID_TAX<>'$PlayerID' ORDER BY Nom ASC";
					else
						$query="SELECT DISTINCT ID,Nom,Longitude,Latitude FROM Lieu WHERE Latitude >=43 AND Latitude <58 AND Longitude <14 AND ID<>'$Base' AND Recce_PlayerID<>'$PlayerID' AND Recce_PlayerID_TAL<>'$PlayerID' AND Recce_PlayerID_TAX<>'$PlayerID' ORDER BY Nom ASC";
					$con=dbconnecti();
					...
					if($result)
					{
						while($data=mysqli_fetch_array($result,MYSQLI_ASSOC)) 
						{
							$Dist=GetDistance(0,0,$Long_base,$Lat_base,$data['Longitude'],$data['Latitude']);
							$Dist_chk=($Dist[0]*2)+200;
							if(($Dist_chk <$Avion1_a or $Dist_chk <$Avion2_a or $Dist_chk <$Avion3_a or $Dist_chk <$Avion4_a or $Dist_chk <$Avion5_a) and $Dist[0] >30)
							{
								$idata=$data['Nom'].' ('.$Dist[0].'km)';
								$Lieux.='<option value='.$data['ID'].'>'.$idata.'</option>';
							}
				...
				else
				{
					...
					{
						if($Front ==3)
							$query="SELECT DISTINCT ID,Nom,Longitude,Latitude FROM Lieu WHERE Longitude >67 AND ID<>'$Base' ORDER BY RAND()";
						elseif($Front ==1)
							$query="SELECT DISTINCT ID,Nom,Longitude,Latitude FROM Lieu WHERE Latitude >=43 AND Latitude <=50.5 AND Longitude >13 AND Longitude <50 AND ID<>'$Base' ORDER BY RAND()";
						elseif($Front ==4)
							$query="SELECT DISTINCT ID,Nom,Longitude,Latitude FROM Lieu WHERE Latitude >50.5 AND Longitude >13 AND Longitude <50 AND ID<>'$Base' ORDER BY RAND()";
						elseif($Front ==5)
							$query="SELECT DISTINCT ID,Nom,Longitude,Latitude FROM Lieu WHERE Latitude >60 AND Longitude >13 AND Longitude <50 AND ID<>'$Base' ORDER BY RAND()";
						elseif($Front ==2)
							$query="SELECT DISTINCT ID,Nom,Longitude,Latitude FROM Lieu WHERE Latitude <43 AND Longitude <50 AND ID<>'$Base' ORDER BY RAND()";
						else
							$query="SELECT DISTINCT ID,Nom,Longitude,Latitude FROM Lieu WHERE Latitude >=43 AND Latitude <58 AND Longitude <14 AND ID<>'$Base' ORDER BY RAND()";
						$con=dbconnecti();
		...
		if($voyage and !$finmission)
		{
			...
			<table class='table'>
				<thead><tr><th>Destination <span><img src='images/help.png' title='La destination est parfois imposée par votre hiérarchie selon la mission choisie. Certaines missions et plus tard certains grades vous permettront de choisir votre destination de vol'></span></th></tr></thead>
				<tr><td><select name='Cible' class='form-control' style='width: 300px'>".$Lieux."</select></td></tr></table>
			<table class='table table-striped'>
				<thead><tr><th>Avion</th><th>Disponibles</th><th title='Distance maximale pouvant être parcourue par votre avion'>Autonomie <span><img src='images/help.png' title='Si plusieurs modèles différents composent votre formation, la valeur la plus faible sera utilisée pour déterminer la distance franchissable.'></span></th><th>Robustesse</th><th>Vitesse max</th><th>Plafond</th><th>Armement</th><th>Soute</th></tr></thead>";
			if($AvionT1_Nbr)
			...
			if($AvionT5_txt)
				$mes.="<tr><td>".$AvionT5_txt." <span><img src='images/help.png' title='Si cet avion est choisi, les pilotes utiliseront le ".$Sqn." 1, puis le 2 et ensuite le 3'></span></td><td>1</td><td>".$Avion5_a."km</td><td>".$Avion5_r."</td><td>".$Avion5_v."</td><td>".$Avion5_pl."</td><td>".$Avion5_ap_txt."</td><td>".$Avion5_Bombe_txt."</td></tr>";
			if($AvionT4_txt)
				$mes.="<tr><td>".$AvionT4_txt." <span><img src='images/help.png' title='Si cet avion est choisi, les pilotes utiliseront le ".$Sqn." 1, puis le 2 et ensuite le 3'></span></td><td>1</td><td>".$Avion4_a."km</td><td>".$Avion4_r."</td><td>".$Avion4_v."</td><td>".$Avion4_pl."</td><td>".$Avion4_ap_txt."</td><td>".$Avion4_Bombe_txt."</td></tr>";
			$mes.="</table>";
			if($Type_Mission ==4 or $Type_Mission ==6 or $Type_Mission ==7 or $Type_Mission ==8 or $Type_Mission ==16 or $Type_Mission ==17 or $Type_Mission ==21 or $Type_Mission ==23 or $Type_Mission ==24 or $Type_Mission ==25 or $Type_Mission ==26)
			{
				...
				WHERE Unit='$Unite' AND Task=0 AND Couverture=0 AND Couverture_Nuit=0 AND Escorte=0 AND Actif=1 AND Moral>0 AND Courage>0 ORDER BY Avancement DESC, Reputation DESC LIMIT 12";
				$con=dbconnecti();
				...
				if($result)
				{
					while($Data=mysqli_fetch_array($result,MYSQLI_ASSOC))
					{
						...
						$pilotes_txt.="<tr><td>".$Data['Nom']."</td><td title='".$Avancement[0]."'><img src='images/grades".$country.$Avancement[1].".png'></td><td>".$Reputation."</td><td>"
				...
				<th>Moral</th><th>Courage</th><th>Choisir <span><img src='images/help.png' title='Plusieurs pilotes peuvent être sélectionnés'></span></th></tr></thead><tbody>".$pilotes_txt."</tbody></table></div>";
			}
