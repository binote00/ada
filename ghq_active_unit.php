...
if(isset($_SESSION['AccountID']) && $OfficierEMID >0)
{
	...
	if($GHQ || $Admin)
	{
        ...
        if($Escadrille && $Typ)
        {
            ...
            if($Avion >0 and $Retraite)
            {
                ...
                Avion1_Mun1=0,Avion2_Mun1=0,Avion3_Mun1=0,Mission_IA=0,CT=0
                WHERE ID='$Escadrille'");
                if(!$Pilotes || $Pilotes <11)
                {
                    ...
                    elseif($country ==UK or $country ==USA)
                    {
                    ...
                    $ok=true;
                if($ok) {
                    $_SESSION['msg_esc']='Vous activez l\'unité <b>'.$Nom.'</b><br>Les pilotes ont été affectés à l\'unité';
                    Pilote_IA::resetPilotsByUnit($Escadrille);
                }
                else
                    $_SESSION['msg_esc']='Vous activez l\'unité <b>'.$Nom.'</b><br>Aucun pilote n\'a été affectés à l\'unité';
                mysqli_close($con);
            }
            else
                $_SESSION['msg_esc_red']='Unité non activée !';
        }
        Output::redirect('index.php?view=em_unites_98');
	}
	else
